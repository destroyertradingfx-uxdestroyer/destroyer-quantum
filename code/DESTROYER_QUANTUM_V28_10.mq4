...
//|  DESTROYER QUANTUM V28.10 - VENI VIDI VICI                        |
...
#property version   "28.10"  // V28.10: Fixed triple GetStrategyIndex conflict, added Sentinel, resized arrays
#property strict

/*
==================================================================================================================
V28.10 CHANGELOG (Built on V28.06 proven base - 50399 profit, PF 1.92, DD 19.40%, 601 trades)
==================================================================================================================

BUG FIXES (critical - these caused V28.09 account blowup):

1. FIXED: Triple GetStrategyIndex conflict (Bug 1 - THE KILLER)
   - GetStrategyIndexByMagic() had COMPLETELY WRONG index assignments
   - Reaper to 0 (should be 4), Phantom to 5 (should be 9), Nexus to 7 (should be 10)
   - MoneyManagement_Quantum() used this function to look up Kelly/heat/trade counts
   - Result: SessionMomentum and Nexus got wrong lot sizes (98 percent profit collapse)
   - Fix: Unified all three mapping functions to match g_perfData[] indices

2. FIXED: Sentinel not in IsOurMagicNumber (Bug 2)
   - Sentinel magic 777015 was missing from IsOurMagicNumber
   - Result: Sentinel trades invisible to EA tracking (197 unaccounted trades)
   - Fix: Added InpSentinel_MagicNumber to IsOurMagicNumber()

3. FIXED: Array size mismatch (Bug 3)
   - g_perfData[18] but all other arrays only [17]
   - Sentinel at index 17 couldn't access Kelly/heat/multiplier arrays
   - Fix: Resized ALL arrays to [18]

4. FIXED: ReconcileFinalPerformance loop bounds
   - Was looping to 17, now loops to 18 for Sentinel
...
5. SENTINEL (magic 777015) — EMA Crossover Trend Follower
   ...
   - Registered in all 3 GetStrategyIndex functions (index 17)
   - Added to GetStrategySpecificRisk (0.8x conservative)
   - Added to MoneyManagement_Quantum fallback path (1.0x)

BASE: V28.06 (unchanged strategy logic - no Phantom Tuesday gap extension,
       no NB relaxation - those caused V28.09 degradation on top of the bugs)
...
==================================================================================================================
   ...
   CHANGES:
   1. CUT LiquiditySweep (9005) - PF 0.84, negative EV (-1439). Disabled.
   2. REVERTED Titan volatility threshold - 0.25 to 0.4. Restored Valkyrie filter.
      V27.27 loosened these and let in 17 garbage trades (PF 2.00 to 0.37).
   ...
   5. ADJUSTED DivergenceMR Hurst threshold - 0.5 to 0.55 (EURUSD H4 rarely < 0.5).
      Extended StructuralRetest retest window - 10 to 20 bars (was too tight on H4).
...
==================================================================================================================
   ...
   2. DYNAMIC TIER CAPS (Performance-Based, Not Hardcoded)
      ...
      - New: PF>=3.0 gives 4.0x, PF>=2.0 gives 3.0x, PF>=1.5 gives 2.5x, PF>=1.2 gives 2.0x, PF<1.0 gives 0.75x
   ...
   3. HEAT SCORE (Capital Allocation Weight)
      ...
      - Maps to risk scaling: 0.0 gives 0.25x, 0.5 gives 1.0x, 1.0 gives 2.0x
   ...
   4. REAPER GRID AMPLIFICATION
      ...
      - Grid max exposure now approx 1.2 lots (was approx 0.24)
      
   5. PORTFOLIO RISK BUDGET RAISED
      - InpMaxTotalRisk_Percent: 5.0% to 8.0% (Kelly-governed needs headroom)
   ...
   DEVELOPED BY: okyyryan + V27.19 Dynamic Kelly Integration
...
==================================================================================================================
   ...
   THE BREAKTHROUGH:
   V25's elastic layer is throttled by V18 indicator rarity (approx 190 signals). To reach 600-900 trades with PF >3.5:
...
==================================================================================================================
   ...
   *** V25 ELASTIC LAYER FEATURES (ALL 4 FIXES INTEGRATED):
   ...
   1. Fix #1 (Marginal VAR)  ->  approx 280 trades
   2. Fix #4 (Complete Re-entries)  ->  approx 450 trades
   3. Fix #2 (Regime Probation)  ->  approx 600 trades
...
==================================================================================================================
   ...
   OBJECTIVE:
   V23's mathematical layers act as filters/governors that stabilize but cap frequency at approx 192 trades.
   ...
   *** V24 EXPANSION FEATURES:
   ...
   CONFIGURATION:
   - input bool InpAlphaExpand = false (V23 mode: stable 192 trades, PF approx 4.0)
...
==================================================================================================================
   ...
   2. Integration Points:
      - Line approx 1476: iBandsOnArray() helper function added
      - Line approx 4910: ExecuteMicrostructureStrategy() function added
      - Line approx 4049: OnTick() hookup for M15 execution
...
==================================================================================================================
   ...
   - Impact: Titan identifies trends approx 3-5 candles earlier
...
==================================================================================================================
   ...
   - Enhancement: Reacts to trends approx 40% faster than EMA
...
datetime g_strategyLockoutUntil[18];                // V28.10: Per-strategy lockout timestamps
...
double   g_strategyMultiplier[18];                 // Dynamic risk multipliers [0-16] per strategy index
...
int      g_stratProfitIdx[18];                     // V28.10: Current circular buffer index per strategy
...
StrategyCooldown g_strategyCooldown[18]; // V28.10: Extended to 18 strategies
// ---
...
void UpdatePerformanceMetrics()
{
    ...
    if(total_trades > 10)
    {
        ...
        for(int i = 0; i < 18 && i < ArraySize(g_perfData); i++) // V28.10: Extended to 18
        {
            if(g_perfData[i].trades > 0)
            {
...
input int     InpSentinel_RSI_Period    = 14;        // RSI period for confirmation
input double  InpSentinel_RSI_BuyLevel  = 45.0;      // RSI must be above this for BUY
input double  InpSentinel_RSI_SellLevel = 55.0;      // RSI must be below this for SELL
input double  InpSentinel_SL_ATR_Mult   = 1.5;       // SL = ATR * this
input double  InpSentinel_TP_RR         = 2.0;       // TP = SL distance * this (Risk:Reward)
...
int OnInit()
{
   ...
   for(int i=0; i<18; i++) // V28.10: Extended to 18 strategy slots (added Sentinel)
   {
   ...
   g_perfData[9].name = "Phantom";
    ...
    g_perfData[17].name = "Sentinel"; // V28.10: New strategy
   // ---
   
   // V13.0 ELITE: Initialize Strategy Cooldown System
   for(int i = 0; i < 18; i++)  // V28.10: Extended to 18 strategies
   {
...
bool IsOurMagicNumber(int magic)
{
    if(magic == InpMagic_MeanReversion || 
       ...
       magic == InpSentinel_MagicNumber)              // V28.10: Sentinel
    {
        return true;
    }
    return false;
...
int GetStrategyIndexFromMagic(int magicNumber) 
{
    ...
    if(magicNumber == InpSentinel_MagicNumber) return 17; // V28.10: Sentinel
...
//+------------------------------------------------------------------+
//| V28.10: Reconcile Final Performance                               |
...
void ReconcileFinalPerformance()
{
   ...
   PerfData reconciledData[18]; // V28.10: Extended to 18 strategies
   for(int i=0; i<18; i++) // V28.10: Extended to 18 strategies
   {
...
int GetStrategyIndex(int magic)
{
   ...
   if(magic == InpSentinel_MagicNumber) return 17; // V28.10: Sentinel EMA Crossover
   return -1;
}
...
//| SL: ATR * 1.5, TP: SL * 2.0 (2:1 R:R)                          |
//+------------------------------------------------------------------+
void ExecuteSentinelStrategy()
{
   if(!InpSentinel_Enabled) return;
   if(Period() != PERIOD_H4) return;
   ...
   double slowEMA_prev = iMA(Symbol(), Period(), InpSentinel_SlowEMA, 0, MODE_EMA, PRICE_CLOSE, 2);
   
   double rsi = iRSI(Symbol(), Period(), InpSentinel_RSI_Period, PRICE_CLOSE, 1);
   ...
   if(bullishCross && rsi > InpSentinel_RSI_BuyLevel && priceAboveSlowEMA)
   {
      ...
      double tpDist = slDist * InpSentinel_TP_RR;
      double slPrice = Bid - slDist;
      double tpPrice = Ask + tpDist;
      
      int ticket = OpenTrade(OP_BUY, lots, Ask, slPrice, tpPrice, "SENTINEL_BUY", InpSentinel_MagicNumber);
      if(ticket > 0)
      {
         if(stratIdx >= 0) g_perfData[stratIdx].trades++;
         LogError(ERROR_INFO, "SENTINEL BUY: EMA" + IntegerToString(InpSentinel_FastEMA) + 
                   " crossed above EMA" + IntegerToString(InpSentinel_SlowEMA) + 
                   " | RSI=" + DoubleToString(rsi, 1) + 
                   " | SL=" + DoubleToString(slPrice, 5) + " TP=" + DoubleToString(tpPrice, 5), "ExecuteSentinelStrategy");
      }
      return;
   }
   ...
   if(bearishCross && rsi < InpSentinel_RSI_SellLevel && priceBelowSlowEMA)
   {
      ...
      int ticket = OpenTrade(OP_SELL, lots, Bid, slPrice, tpPrice, "SENTINEL_SELL", InpSentinel_MagicNumber);
      if(ticket > 0)
      {
         if(stratIdx >= 0) g_perfData[stratIdx].trades++;
         LogError(ERROR_INFO, "SENTINEL SELL: EMA" + IntegerToString(InpSentinel_FastEMA) + 
                   " crossed below EMA" + IntegerToString(InpSentinel_SlowEMA) + 
                   " | RSI=" + DoubleToString(rsi, 1) + 
                   " | SL=" + DoubleToString(slPrice, 5) + " TP=" + DoubleToString(tpPrice, 5), "ExecuteSentinelStrategy");
      }
      return;
...
int GetStrategyIndexByMagic(int magicNumber)
{
   ...
   if(magicNumber == InpVortex_MagicNumber) return 11; // Vortex (shares index with MathReversal)
