...
//|  DESTROYER QUANTUM V28.00 - FULL OVERHAUL: DD Tightening,       |
...
extern double InpReaper_RegimeADX       = 30.0;       // V27.1: ADX threshold for regime suppression
...
void ProcessReaperBasket(int magic_number, int order_type)
{
   ...
   // Extra safety: If ADX > 25 and we already have 4+ levels, stop adding
   if(adx > 25.0 && basket_levels >= 4)
   {
...
public:
    ...
    bool ApproveTrade(int strategyIndex, double riskAmount, double conviction) {
        ...
        if(!InpDisable_VAR_Limiter && riskAmount > m_portfolioVAR * 0.1) {
