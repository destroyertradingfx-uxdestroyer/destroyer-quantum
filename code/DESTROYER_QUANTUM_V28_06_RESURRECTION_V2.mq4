...
void ExecuteAsianBreakoutStrategy()
{
   ...
   // Only trade at London open: bar starting at 08:00 UTC
   // On H4, bars open at 00, 04, 08, 12, 16, 20 UTC
   // We want the bar that opens at 08:00 UTC
   int barHour = TimeHour(Time[0]);
   if(barHour != 8) return;

   // Get Asian session range: High and Low from 00:00-08:00 UTC
   // On H4, that's the 2 bars before the 08:00 bar (00:00 bar and 04:00 bar)
   double asianHigh = MathMax(High[1], High[2]);
   double asianLow  = MathMin(Low[1], Low[2]);
   double asianRange = asianHigh - asianLow;

   // Goldilocks filter: range must be 0.3x-1.5x ATR(14)
   double atr = iATR(NULL, PERIOD_H4, 14, 1);
   if(atr <= 0) return;

   double rangeRatio = asianRange / atr;
   if(rangeRatio < InpAsianBreakout_RangeMinATR) return; // Too narrow ? no volatility
   if(rangeRatio > InpAsianBreakout_RangeMaxATR) return; // Too wide ? already moved

   // ADX filter: need some trend
   double adx = iADX(NULL, PERIOD_H4, InpAsianBreakout_ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
   ...
   // Breakout signal
   double close1 = Close[1]; // Last closed bar
   bool buySignal  = (close1 > asianHigh);
   bool sellSignal = (close1 < asianLow);
   ...
   double slDist = atr * InpAsianBreakout_ATR_SL_Mult;
   double tpDist = atr * InpAsianBreakout_ATR_TP_Mult;
