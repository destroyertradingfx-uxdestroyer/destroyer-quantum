...
==================================================================================================================
==================================================================================================================
   ### V28.06 SIGMA  CRITICAL BUG FIXES ###
==================================================================================================================
   CODENAME: SIGMA
   DATE: 2026-05-25
   STATUS: BACKTEST PENDING
   
   BUILT ON: OMEGA (all bottlenecks removed)
   ADDS: Critical bug fixes + final optimizations
   
   CRITICAL BUGS FIXED:
   1. DD Level Ordering  Level 3 was dead code (Level 4 at 12% caught everything before Level 3 at 14%)
      Fix: Level 4 now at 18%, Level 3 at 14% (as intended)
   2. CountOpenTrades missing 6 strategies  Vortex, RegimeShift, SessionMomentum, DivergenceMR,
      LiquiditySweep, StructuralRetest were invisible to portfolio capacity management
   3. sxRoomAvailable gate  Even with SX disabled, gate blocked Apex/Phantom/Nexus if SX had open trades
   
   BOTTLENECK FIXES:
   4. Phantom: 2 concurrent (was 1), MinGap 3.0 pips (was 5.0)
   5. NoiseBreakout: 4 concurrent (was 3)
   6. Nexus Reaper opposition gate REMOVED (consistent with Apex OMEGA fix)
   
   PARAMETER OPTIMIZATIONS:
   7. Base risk 2.0%  2.5%
   8. Max loss per trade $800  $1200
   
   PROJECTED: $155K-$210K profit (combined with all previous fixes)
...
extern double InpBase_Risk_Percent    = 2.5;    // SIGMA: Raised from 2.0% for $170K target// TITAN: Raised from 1.5% for $170K target         // AGGRESSIVE: Raised from 1.0% for maximum growth         // V27.27: Raised from 0.5% for $100K target
...
void ManageDrawdownExposure_V2()
{
   ...
   if(ddPercent >= 18.0) currentDDLevel = 4;  // SIGMA: Emergency at 18% (was 12%, Level 3 was dead)
...
// }
   ...
   if(InpApex_Enabled)  // SIGMA: Removed sxRoomAvailable gate
   {
      ExecuteApexStrategy();
      if(CountOpenTrades() >= InpMaxOpenTrades) { if(!IsOptimization()) UpdateDashboard_StaticV8_6(); return; }
   }
   ...
   if(InpPhantom_Enabled)  // SIGMA: Removed sxRoomAvailable gate
   {
   ...
   if(InpNexus_Enabled)  // SIGMA: Removed sxRoomAvailable gate
   {
...
void ExecuteNexusStrategy()
{
   ...
   // SIGMA: Removed Reaper opposition gate  Nexus is independent
      ...
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
      {
         if(OrderSymbol() == Symbol())
         {
            ...
            if(magic == InpMagic_MeanReversion   ||
               ...
               magic == 9001                      ||   // Vortex
               magic == 9002                      ||   // RegimeShift
               magic == 9003                      ||   // SessionMomentum
               magic == 9004                      ||   // DivergenceMR
               magic == 9005                      ||   // LiquiditySweep
               magic == 9006)                          // StructuralRetest
            {
               count++;
            }
         }
...
void CalculateRollingKelly(int idx)
{
   ...
   g_stratSharpeProxy[idx] = (stddev > 0) ? meanProfit / stddev : 0;  // SIGMA FIX: prevent zero divide
