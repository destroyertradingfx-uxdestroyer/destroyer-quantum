...
//+------------------------------------------------------------------+
//| A-TIER STRATEGY 1: TBM -- Session Liquidity Sweep Reversal       |
//| Magic 9010 | Edge: ICT killzone liquidity raid + reversal        |
//+------------------------------------------------------------------+
void ExecuteTBM(double lotSize, double rrRatio)
{
   int magic = 9010;
   if(CountOpenTradesByMagic(magic) >= 1) return;

   double spread = MarketInfo(Symbol(), MODE_SPREAD);
   if(spread > InpMax_Spread_Pips * 10) return;

   // Only fire during London killzone (07:00-09:59) or NY killzone (13:00-15:59)
   int hr = TimeHour(TimeCurrent());
   if(!((hr >= 7 && hr <= 9) || (hr >= 13 && hr <= 15))) return;

   // Per-bar guard: only 1 signal attempt per completed H4 bar
   static datetime lastBarTBM = 0;
   datetime curBar = iTime(NULL, PERIOD_H4, 1);
   if(curBar == lastBarTBM) return;

   // Structural trend filter: EMA50 + EMA200 on H4
   double ema50  = iMA(NULL, PERIOD_H4, 50,  0, MODE_EMA, PRICE_CLOSE, 1);
   double ema200 = iMA(NULL, PERIOD_H4, 200, 0, MODE_EMA, PRICE_CLOSE, 1);

   // RSI: avoid entering at extremes
   double rsi = iRSI(NULL, PERIOD_H4, 14, PRICE_CLOSE, 1);

   // ATR: dynamic SL sizing
   ...
   double h2 = High[2],  l2 = Low[2];
   double range1 = h1 - l1;
   if(range1 <= 0) return;

   // BUY: sweep below Bar[2] low, recovery, bullish close with strength
   if(ema50 > ema200 && rsi > 30 && rsi < 60)
   {
      bool swept     = l1 < l2;
      bool recovery  = c1 > l2;
      bool bullClose = c1 > o1;
      bool strength  = (c1 - l1) / range1 > 0.50;

      if(swept && recovery && bullClose && strength)
      {
   ...
   if(ema50 < ema200 && rsi > 40 && rsi < 70)
   {
      ...
      bool strength   = (h1 - c1) / range1 > 0.50;

      if(swept && recovery && bearClose && strength)
      {
...
void ExecutePO3(double lotSize, double rrRatio)
{
   ...
   // BUY: bullish trend + RSI momentum zone + expansion + top close
   if(ema50 > ema200 && rsi >= 48 && rsi <= 68 && isExpansion && strongBody)
   {
      bool bullClose = c1 > o1;
      ...
      {
         double sl   = l1 - atr * 0.40 - 10 * Point;
         double dist = Ask - sl;
         if(dist <= 0) return;
         double tp   = Ask + dist * rrRatio;
         double lots = MoneyManagement_Quantum(magic, InpBase_Risk_Percent);
         ...
         int ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, sl, tp,
                                "PO3_BUY", magic, 0, clrGreen);
         if(ticket > 0) { lastBarPO3 = curBar; if(idx >= 0) g_perfData[idx].trades++; }
         return;
      }
   }

   // SELL: bearish trend + RSI bearish zone + expansion + bottom close
   if(ema50 < ema200 && rsi >= 32 && rsi <= 52 && isExpansion && strongBody)
   {
      bool bearClose   = c1 < o1;
      ...
      {
         double sl   = h1 + atr * 0.40 + 10 * Point;
         double dist = sl - Bid;
         if(dist <= 0) return;
         double tp   = Bid - dist * rrRatio;
         double lots = MoneyManagement_Quantum(magic, InpBase_Risk_Percent);
...
void ExecuteFractalModel(double lotSize, double rrRatio)
{
   ...
   double rsi0 = iRSI(NULL, PERIOD_H4, 14, PRICE_CLOSE, 0);

   double atr = iATR(NULL, PERIOD_H4, 14, 1);
   if(atr <= 0) return;

   // Scan last 15 bars for confirmed 3-bar fractal low/high
   double fractalLow  = -1;
   double fractalHigh = -1;
   ...
   // BUY: bullish trend + RSI rising from pullback + price near fractal low
   if(ema50 > ema200 && fractalLow > 0 && rsi1 > 35 && rsi1 < 55 && rsi0 > rsi1)
   {
      bool nearFractal  = MathAbs(Ask - fractalLow) <= atr * 0.60;
      bool aboveFractal = Ask > fractalLow;

      if(nearFractal && aboveFractal)
      {
         double sl   = fractalLow - atr * 0.40 - 10 * Point;
         double dist = Ask - sl;
         if(dist <= 0) return;
         double tp   = Ask + dist * rrRatio;
         double lots = MoneyManagement_Quantum(magic, InpBase_Risk_Percent);
         ...
         int ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, sl, tp,
                                "FRAC_BUY", magic, 0, clrGreen);
         if(ticket > 0) { lastBarFractal = curBar; if(idx >= 0) g_perfData[idx].trades++; }
         return;
      }
   }

   // SELL: bearish trend + RSI falling + price near fractal high
   if(ema50 < ema200 && fractalHigh > 0 && rsi1 > 45 && rsi1 < 65 && rsi0 < rsi1)
   {
      bool nearFractal  = MathAbs(Bid - fractalHigh) <= atr * 0.60;
      bool belowFractal = Bid < fractalHigh;

      if(nearFractal && belowFractal)
      {
         double sl   = fractalHigh + atr * 0.40 + 10 * Point;
         double dist = sl - Bid;
         if(dist <= 0) return;
         double tp   = Bid - dist * rrRatio;
         double lots = MoneyManagement_Quantum(magic, InpBase_Risk_Percent);
...
void ExecuteCRT(double lotSize, double rrRatio)
{
   ...
   // Only fire during London open window: 07:00-09:59
   int hr = TimeHour(TimeCurrent());
   if(hr < 7 || hr > 9) return;
   ...
   // Asian range: look back 3 H4 bars (12 hours) from bar[1]
   int asianBars = 3;
   int   hiIdx = iHighest(NULL, PERIOD_H4, MODE_HIGH, asianBars, 1);
   int   loIdx = iLowest (NULL, PERIOD_H4, MODE_LOW,  asianBars, 1);
   double asianHigh  = High[hiIdx];
   double asianLow   = Low[loIdx];
   double asianRange = asianHigh - asianLow;

   // Range must be meaningful: 0.4-2.0 ATR
   if(asianRange < atr * 0.40) return;
   if(asianRange > atr * 2.00) return;

   // BUY: bullish trend + RSI momentum + breakout above asian high
   if(ema50 > ema200 && rsi > 50 && rsi < 70)
   {
      bool breakoutBar = Close[1] > asianHigh;
      bool priceAbove  = Ask > asianHigh;
      bool notTooFar   = Ask < asianHigh + atr;

      if(breakoutBar && priceAbove && notTooFar)
      {
         double sl   = asianLow - atr * 0.30 - 10 * Point;
         double dist = Ask - sl;
         if(dist <= 0) return;
         double tp   = Ask + dist * rrRatio;
         double lots = MoneyManagement_Quantum(magic, InpBase_Risk_Percent);
         ...
         int ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, sl, tp,
                                "CRT_BUY", magic, 0, clrGreen);
         if(ticket > 0) { lastBarCRT = curBar; if(idx >= 0) g_perfData[idx].trades++; }
         return;
      }
   }

   // SELL: bearish trend + RSI bearish momentum + breakout below asian low
   if(ema50 < ema200 && rsi > 30 && rsi < 50)
   {
      bool breakoutBar = Close[1] < asianLow;
      bool priceBelow  = Bid < asianLow;
      bool notTooFar   = Bid > asianLow - atr;

      if(breakoutBar && priceBelow && notTooFar)
      {
         double sl   = asianHigh + atr * 0.30 + 10 * Point;
         double dist = sl - Bid;
         if(dist <= 0) return;
         double tp   = Bid - dist * rrRatio;
         double lots = MoneyManagement_Quantum(magic, InpBase_Risk_Percent);
