//+------------------------------------------------------------------+
//|        DESTROYER_QUANTUM_V29_00.mq4                              |
//|                    Copyright 2026, Quantum Leap Analytics        |
//|  DESTROYER QUANTUM V29.00 - VENI VIDI VICI                       |
//|  7 Institutional Enhancements: MTF + KillZone + Chandelier +    |
//|  VolSizing + OrderBlock + FVG + ORB                             |
//|                     https://github.com/okyyryan                  |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Quantum Leap Analytics"
#property link      "https://github.com/okyyryan"
#property version   "29.00"  // V28.04: Cut LiquiditySweep, reverted Titan, fixed lot sizing, adjusted DivergenceMR+StructuralRetest
#property strict
...
double   g_strategyMultiplier[20];                 // Dynamic risk multipliers [0-16] per strategy index
...
void LogError(ERROR_LEVEL level, string message, string function = "", int line = 0)
{
    ...
    if(TimeCurrent() - lastErrorCheck > 60) // Check every minute
    {
        ...
        if(errorCount > 10)
        {
            Print("WARNING:  WARNING: High critical error rate detected - " + IntegerToString(errorCount) + " errors in last hour");
        }
...
void UpdatePerformanceMetrics()
{
    ...
    if(total_trades > 10)
    {
        ...
        for(int i = 0; i < 20 && i < ArraySize(g_perfData); i++) // V29.00: Extended to 20
        {
            if(g_perfData[i].trades > 0)
            {
...
//                                                                        
// V29.00 ENHANCEMENTS   7 INSTITUTIONAL-GRADE IMPROVEMENTS
// VENI VIDI VICI
//                                                                        

//    SYSTEM 1: MULTI-TIMEFRAME TREND ALIGNMENT   
sinput string Inp_Header_MTF = "====== V29.00: MULTI-TIMEFRAME TREND ALIGNMENT ======";
input bool    InpMTF_Enabled           = true;        // Enable Multi-Timeframe Filter
input int     InpMTF_D1_EMA_Fast       = 50;          // D1 fast EMA period
input int     InpMTF_D1_EMA_Slow       = 200;         // D1 slow EMA period
input int     InpMTF_H4_EMA_Period     = 50;          // H4 EMA period
input int     InpMTF_H1_EMA_Period     = 20;          // H1 EMA period
input int     InpMTF_MinAlignment      = 2;           // Min TFs aligned (2 of 3)

//    SYSTEM 2: SESSION KILL ZONES   
sinput string Inp_Header_KillZone = "====== V29.00: SESSION KILL ZONES ======";
input bool    InpKillZone_Enabled      = true;        // Enable Kill Zone Filter
input int     InpKillZone_LondonStart  = 8;           // London open (GMT hour)
input int     InpKillZone_LondonEnd    = 12;          // London session end (GMT hour)
input int     InpKillZone_NYStart      = 13;          // NY open (GMT hour)
input int     InpKillZone_NYEnd        = 17;          // NY close (GMT hour)

// -- SYSTEM 3: ADVANCED TRAILING STOPS (CHANDELIER) --
// Uses existing InpChandelier_Period and InpChandelier_Multiplier (line 1151)
sinput string Inp_Header_Chandelier = "====== V29.00: CHANDELIER TRAILING STOP ======";
input bool    InpChandelier_Enabled    = true;        // Enable Chandelier Trail
input int     InpChandelier_Lookback   = 22;          // Highest high / Lowest low lookback

//    SYSTEM 4: ADAPTIVE VOLATILITY POSITION SIZING   
sinput string Inp_Header_VolSizing = "====== V29.00: ADAPTIVE VOLATILITY SIZING ======";
input bool    InpVolSizing_Enabled     = true;        // Enable Vol-Adjusted Position Sizing
input int     InpVolSizing_ATR_Period  = 14;          // ATR period for vol calculation
input int     InpVolSizing_AVG_Period  = 50;          // Bars for average ATR baseline
input double  InpVolSizing_MinMult     = 0.25;        // Min lot multiplier (high vol)
input double  InpVolSizing_MaxMult     = 2.0;         // Max lot multiplier (low vol)

//    SYSTEM 5: ICT ORDER BLOCKS   
sinput string Inp_Header_OrderBlock = "====== V29.00: ICT ORDER BLOCK ======";
input bool    InpOrderBlock_Enabled     = true;       // Enable Order Block Strategy
input int     InpOrderBlock_MagicNumber = 9008;       // Magic number for Order Block
input int     InpOrderBlock_Lookback    = 50;         // Bars to scan for OBs
input double  InpOrderBlock_DisplacementMult = 2.0;   // Displacement = Nx avg body
input double  InpOrderBlock_MinSizePips = 5.0;        // Minimum OB size in pips
input double  InpOrderBlock_MaxSizePips = 100.0;      // Maximum OB size in pips
input int     InpOrderBlock_MaxAge_Bars = 30;         // Max OB age in H4 bars
input double  InpOrderBlock_ATR_SL_Mult = 1.5;        // ATR multiplier for SL
input double  InpOrderBlock_TP_RR       = 2.5;        // Risk:Reward ratio for TP
input int     InpOrderBlock_MaxConcurrent = 1;        // Max concurrent OB trades

//    SYSTEM 6: FAIR VALUE GAP (FVG) + LIQUIDITY SWEEP   
sinput string Inp_Header_FVG = "====== V29.00: FAIR VALUE GAP (FVG) ======";
...
input double  InpFVG_MaxSizePips       = 80.0;        // Maximum FVG size (skip breakaway)
input int     InpFVG_MaxFVGAge_Bars    = 20;          // Max age of FVG in H4 bars
input int     InpFVG_SweepLookback     = 6;           // H4 bars for session high/low
input double  InpFVG_SL_Buffer_ATR     = 0.3;         // ATR buffer for SL
input double  InpFVG_TP_RR             = 2.5;         // Risk:Reward for TP
input int     InpFVG_MaxSweepRetrace   = 5;           // Max bars between sweep and entry
...
//    SYSTEM 7: 8AM CANDLE OPENING RANGE BREAKOUT (ORB)   
sinput string Inp_Header_ORB = "====== V29.00: 8AM CANDLE ORB ======";
input bool    InpORB_Enabled           = true;        // Enable ORB Strategy
input int     InpORB_MagicNumber       = 9009;        // Magic number for ORB
input int     InpORB_MaxConcurrent     = 1;           // Max concurrent ORB trades
input int     InpORB_RangeStartHour    = 8;           // Server hour: range candle start
input int     InpORB_RangeBars         = 1;           // H4 bars for range (1 = first bar)
input int     InpORB_EntryStartHour    = 12;          // Server hour: start looking for entry
input int     InpORB_EntryEndHour      = 20;          // Server hour: stop looking for entry
input double  InpORB_SL_Pips           = 30.0;        // Stop loss in pips
input double  InpORB_TP_Pips           = 90.0;        // Take profit in pips (3:1 R:R)
input bool    InpORB_UseMidpoint       = true;        // Use midpoint as entry level
input bool    InpORB_RequireEMAFilter  = true;        // Require EMA trend alignment
input int     InpORB_EMA_Period        = 50;          // EMA period for trend filter
...
int OnInit()
{
   ...
   g_perfData[9].name = "Phantom";
    ...
    g_perfData[17].name = "OrderBlock";       // V29.00: ICT Order Block
    g_perfData[18].name = "FairValueGap";     // V29.00: FVG + Liquidity Sweep
    g_perfData[19].name = "OpeningRange";     // V29.00: 8AM Candle ORB
   // ---
   
   // V13.0 ELITE: Initialize Strategy Cooldown System
   for(int i = 0; i < 20; i++)  // V29.00: Extended to 20 strategies
   {
...
//                                                                        
// V29.00: MULTI-TIMEFRAME TREND ALIGNMENT
// Moskowitz, Ooi, Pedersen (2012)   "Time Series Momentum"
//                                                                        
int GetMultiTimeframeBias()
{
   if(!InpMTF_Enabled) return -1; // -1 = no filter, allow all
   
   // D1: Primary trend direction
   double d1_emaFast = iMA(Symbol(), PERIOD_D1, InpMTF_D1_EMA_Fast, 0, MODE_EMA, PRICE_CLOSE, 0);
   double d1_emaSlow = iMA(Symbol(), PERIOD_D1, InpMTF_D1_EMA_Slow, 0, MODE_EMA, PRICE_CLOSE, 0);
   double d1_close = iClose(Symbol(), PERIOD_D1, 0);
   
   bool d1_bullish = (d1_close > d1_emaFast && d1_emaFast > d1_emaSlow);
   bool d1_bearish = (d1_close < d1_emaFast && d1_emaFast < d1_emaSlow);
   
   // H4: Signal timeframe
   double h4_ema = iMA(Symbol(), PERIOD_H4, InpMTF_H4_EMA_Period, 0, MODE_EMA, PRICE_CLOSE, 0);
   bool h4_bullish = (Close[0] > h4_ema);
   bool h4_bearish = (Close[0] < h4_ema);
   
   // H1: Entry precision
   double h1_ema = iMA(Symbol(), PERIOD_H1, InpMTF_H1_EMA_Period, 0, MODE_EMA, PRICE_CLOSE, 0);
   bool h1_bullish = (iClose(Symbol(), PERIOD_H1, 0) > h1_ema);
   bool h1_bearish = (iClose(Symbol(), PERIOD_H1, 0) < h1_ema);
   
   // Alignment score: 0-3
   int bullScore = (d1_bullish ? 1 : 0) + (h4_bullish ? 1 : 0) + (h1_bullish ? 1 : 0);
   int bearScore = (d1_bearish ? 1 : 0) + (h4_bearish ? 1 : 0) + (h1_bearish ? 1 : 0);
   
   if(bullScore >= InpMTF_MinAlignment) return OP_BUY;
   if(bearScore >= InpMTF_MinAlignment) return OP_SELL;
   return -1; // No alignment   block trade
}

//                                                                        
// V29.00: SESSION KILL ZONES
// ICT Kill Zones   London/NY institutional trading hours
//                                                                        
bool IsInKillZone()
{
   if(!InpKillZone_Enabled) return true; // No filter
   
   int gmtHour = TimeHour(TimeCurrent()) - InpServerUTCOffset;
   if(gmtHour < 0) gmtHour += 24;
   
   // London Kill Zone
   if(gmtHour >= InpKillZone_LondonStart && gmtHour < InpKillZone_LondonEnd) return true;
   
   // NY Kill Zone (includes London-NY overlap)
   if(gmtHour >= InpKillZone_NYStart && gmtHour < InpKillZone_NYEnd) return true;
   
   return false;
}

//                                                                        
// V29.00: CHANDELIER TRAILING STOP (replaces basic ATR trail)
// Turtle Traders method   trail from highest high / lowest low
//                                                                        
bool ApplyChandelierTrail(int ticket)
{
   if(!InpChandelier_Enabled) return false;
   if(!OrderSelect(ticket, SELECT_BY_TICKET)) return false;
   
   double atr = iATR(Symbol(), Period(), InpChandelier_Period, 0);
   if(atr <= 0) return false;
   
   double trailDist = atr * InpChandelier_Multiplier;
   double newSL = 0;
   
   if(OrderType() == OP_BUY)
   {
      double highestHigh = High[iHighest(Symbol(), Period(), MODE_HIGH, InpChandelier_Lookback, 1)];
      newSL = highestHigh - trailDist;
      if(newSL <= OrderStopLoss() || newSL >= Bid) return true; // No improvement or invalid
   }
   else if(OrderType() == OP_SELL)
   {
      double lowestLow = Low[iLowest(Symbol(), Period(), MODE_LOW, InpChandelier_Lookback, 1)];
      newSL = lowestLow + trailDist;
      if(newSL >= OrderStopLoss() || newSL <= Ask) return true; // No improvement or invalid
   }
   else return false;
   
   newSL = NormalizeDouble(newSL, Digits);
   bool result = OrderModify(ticket, OrderOpenPrice(), newSL, OrderTakeProfit(), 0, clrGold);
   if(!result)
      LogError(ERROR_WARNING, "Chandelier trail failed. Ticket: " + IntegerToString(ticket) +
               " Err: " + IntegerToString(GetLastError()), "ApplyChandelierTrail");
   return result;
}

//                                                                        
// V29.00: ADAPTIVE VOLATILITY POSITION SIZING
// Inverse vol: smaller lots in high vol, larger in low vol
//                                                                        
double GetVolatilityMultiplier()
{
   if(!InpVolSizing_Enabled) return 1.0;
   
   double currentATR = iATR(Symbol(), PERIOD_H4, InpVolSizing_ATR_Period, 0);
   if(currentATR <= 0) return 1.0;
   
   double avgATR = 0;
   for(int i = 1; i <= InpVolSizing_AVG_Period; i++)
      avgATR += iATR(Symbol(), PERIOD_H4, InpVolSizing_ATR_Period, i);
   avgATR /= InpVolSizing_AVG_Period;
   ...
   // Clamp
   volMult = MathMax(InpVolSizing_MinMult, MathMin(InpVolSizing_MaxMult, volMult));
   
   return volMult;
}

//                                                                        
// V29.00: ICT ORDER BLOCK STRATEGY
// Institutional supply/demand zones from displacement candles
//                                                                        
struct OBZone {
   double high;
   double low;
   int direction; // OP_BUY = demand, OP_SELL = supply
   datetime time;
   bool valid;
};

#define MAX_OB_STORED 20
OBZone g_obStore[MAX_OB_STORED];
int g_obCount = 0;
datetime g_lastOBScanTime = 0;

void AddOBZone(double high, double low, int direction, datetime time)
{
   // Dedup
   for(int i = 0; i < g_obCount; i++)
   {
      if(g_obStore[i].time == time && g_obStore[i].direction == direction) return;
   }
   
   // Size filter
   double sizePips = (high - low) / (Point * 10);
   if(sizePips < InpOrderBlock_MinSizePips || sizePips > InpOrderBlock_MaxSizePips) return;
   
   // FIFO if at capacity
   if(g_obCount >= MAX_OB_STORED)
   {
      for(int j = 0; j < MAX_OB_STORED - 1; j++) g_obStore[j] = g_obStore[j + 1];
      g_obCount = MAX_OB_STORED - 1;
   }
   
   g_obStore[g_obCount].high = high;
   g_obStore[g_obCount].low = low;
   g_obStore[g_obCount].direction = direction;
   g_obStore[g_obCount].time = time;
   g_obStore[g_obCount].valid = true;
   g_obCount++;
}

void ScanForOrderBlocks()
{
   if(Time[0] == g_lastOBScanTime) return;
   g_lastOBScanTime = Time[0];
   
   // Calculate average body size
   double avgBody = 0;
   for(int j = 3; j <= 12; j++) avgBody += MathAbs(Close[j] - Open[j]);
   avgBody /= 10.0;
   if(avgBody <= 0) return;
   
   for(int i = 2; i <= InpOrderBlock_Lookback; i++)
   {
      double body = MathAbs(Close[i] - Open[i]);
      
      // Displacement: strong move (2x+ avg body)
      if(body < InpOrderBlock_DisplacementMult * avgBody) continue;
...
void CleanupOBStore()
{
   ...
   for(int i = 0; i < g_obCount; i++)
   {
      if(!g_obStore[i].valid) continue;
      int age = iBarShift(Symbol(), PERIOD_H4, g_obStore[i].time);
      if(age > InpOrderBlock_MaxAge_Bars) continue;
      if(writeIdx != i) g_obStore[writeIdx] = g_obStore[i];
      writeIdx++;
   }
   g_obCount = writeIdx;
}

void ExecuteOrderBlockStrategy()
{
   if(!InpOrderBlock_Enabled) return;
   if(Period() != PERIOD_H4) return;
   if(CountOpenTrades(InpOrderBlock_MagicNumber) >= InpOrderBlock_MaxConcurrent) return;
   if(!IsStrategyHealthy(InpOrderBlock_MagicNumber)) return;
   ...
   int stratIdx = GetStrategyIndex(InpOrderBlock_MagicNumber);
   
   for(int i = 0; i < g_obCount; i++)
   {
      if(!g_obStore[i].valid) continue;
      
      // Demand zone: price returns to zone -> BUY
      if(g_obStore[i].direction == OP_BUY)
      {
         if(Ask >= g_obStore[i].low && Ask <= g_obStore[i].high)
         {
            double slPrice = g_obStore[i].low - (atr * InpOrderBlock_ATR_SL_Mult);
            double slDist = Ask - slPrice;
            if(slDist / atr < 0.5 || slDist / atr > 4.0) continue;
            double tpPrice = Ask + (slDist * InpOrderBlock_TP_RR);
            
            double lots = MoneyManagement_Quantum(InpOrderBlock_MagicNumber, InpBase_Risk_Percent);
            if(lots <= 0) continue;
            
            int ticket = OpenTrade(OP_BUY, lots, Ask, slPrice, tpPrice,
                                   "OB_BUY", InpOrderBlock_MagicNumber);
            if(ticket > 0)
            {
               g_obStore[i].valid = false; // Consume
               if(stratIdx >= 0) g_perfData[stratIdx].trades++;
               LogError(ERROR_INFO, "OB BUY: Zone[" + DoubleToString(g_obStore[i].low,5) + "-" +
                        DoubleToString(g_obStore[i].high,5) + "]", "ExecuteOrderBlockStrategy");
            }
            return;
         }
      }
      // Supply zone: price returns to zone -> SELL
      else if(g_obStore[i].direction == OP_SELL)
      {
         if(Bid >= g_obStore[i].low && Bid <= g_obStore[i].high)
         {
            double slPrice = g_obStore[i].high + (atr * InpOrderBlock_ATR_SL_Mult);
            double slDist = slPrice - Bid;
            if(slDist / atr < 0.5 || slDist / atr > 4.0) continue;
            double tpPrice = Bid - (slDist * InpOrderBlock_TP_RR);
            
            double lots = MoneyManagement_Quantum(InpOrderBlock_MagicNumber, InpBase_Risk_Percent);
            if(lots <= 0) continue;
            
            int ticket = OpenTrade(OP_SELL, lots, Bid, slPrice, tpPrice,
                                   "OB_SELL", InpOrderBlock_MagicNumber);
            if(ticket > 0)
            {
               g_obStore[i].valid = false;
               if(stratIdx >= 0) g_perfData[stratIdx].trades++;
               LogError(ERROR_INFO, "OB SELL: Zone[" + DoubleToString(g_obStore[i].low,5) + "-" +
                        DoubleToString(g_obStore[i].high,5) + "]", "ExecuteOrderBlockStrategy");
            }
            return;
         }
...
//                                                                        
// V29.00: FAIR VALUE GAP (FVG) + LIQUIDITY SWEEP STRATEGY
// MAC Model   FVG rebalancing after liquidity sweeps
//                                                                        
struct FVGRecord {
   double top;
   double bottom;
   ...
   bool mitigated;
   int sweepConfirmed; // 0=none, +1=bullish, -1=bearish
   datetime sweepTime;
};
...
void AddFVG(double top, double bottom, datetime barTime, int direction)
{
   ...
   {
      if(g_fvgStore[i].barTime == barTime && g_fvgStore[i].direction == direction) return;
   }
   
   double fvgSizePips = (top - bottom) / (Point * 10);
   if(fvgSizePips < InpFVG_MinSizePips || fvgSizePips > InpFVG_MaxSizePips) return;
   
   if(g_fvgCount >= MAX_FVG_STORED)
   {
      for(int j = 0; j < MAX_FVG_STORED - 1; j++) g_fvgStore[j] = g_fvgStore[j + 1];
...
void UpdateFVG_Mitigation()
{
   for(int i = 0; i < g_fvgCount; i++)
   {
      ...
      if(g_fvgStore[i].direction == 1 && Low[0] <= g_fvgStore[i].top)
         g_fvgStore[i].mitigated = true;
      else if(g_fvgStore[i].direction == -1 && High[0] >= g_fvgStore[i].bottom)
         g_fvgStore[i].mitigated = true;
   }
}
...
void CleanupFVGStore()
{
   ...
   for(int i = 0; i < g_fvgCount; i++)
   {
      ...
      if(writeIdx != i) g_fvgStore[writeIdx] = g_fvgStore[i];
      writeIdx++;
   }
   g_fvgCount = writeIdx;
}
...
void ScanForNewFVGs()
{
   ...
   for(int shift = 1; shift <= 48; shift++)
   {
      ...
      if(Low[shift + 2] > High[shift])
         AddFVG(Low[shift + 2], High[shift], Time[shift + 1], +1);
      
      // Bearish FVG: gap down
      if(High[shift + 2] < Low[shift])
         AddFVG(Low[shift], High[shift + 2], Time[shift + 1], -1);
   }
}
...
void DetectLiquiditySweeps()
{
   ...
   for(int i = 2; i <= InpFVG_SweepLookback + 1; i++)
   {
      if(High[i] > sessionHigh) sessionHigh = High[i];
      if(Low[i] < sessionLow) sessionLow = Low[i];
   }
   
   // Bearish sweep: swept above key level but closed back below
   bool sweptAbove = (High[1] > prevDayHigh && Close[1] < prevDayHigh) ||
                     (High[1] > sessionHigh && Close[1] < sessionHigh);
   ...
   // Bullish sweep: swept below key level but closed back above
   bool sweptBelow = (Low[1] < prevDayLow && Close[1] > prevDayLow) ||
                     (Low[1] < sessionLow && Close[1] > sessionLow);
      ...
      if(g_fvgStore[i].direction == 1 && g_fvgStore[i].sweepConfirmed == 1)
      {
         if(Ask >= g_fvgStore[i].bottom && Ask <= g_fvgStore[i].top)
         {
            ...
            if(ticket > 0)
            {
               g_fvgStore[i].mitigated = true;
               if(stratIdx >= 0) g_perfData[stratIdx].trades++;
               LogError(ERROR_INFO, "FVG BUY: Zone[" + DoubleToString(g_fvgStore[i].bottom,5) + "-" +
                        DoubleToString(g_fvgStore[i].top,5) + "]", "ExecuteFVGStrategy");
            }
            return;
         }
      }
      
      // SELL: Bearish FVG + bearish sweep + price inside zone
      if(g_fvgStore[i].direction == -1 && g_fvgStore[i].sweepConfirmed == -1)
      {
         if(Bid >= g_fvgStore[i].bottom && Bid <= g_fvgStore[i].top)
         {
            ...
            if(ticket > 0)
            {
               g_fvgStore[i].mitigated = true;
               if(stratIdx >= 0) g_perfData[stratIdx].trades++;
               LogError(ERROR_INFO, "FVG SELL: Zone[" + DoubleToString(g_fvgStore[i].bottom,5) + "-" +
                        DoubleToString(g_fvgStore[i].top,5) + "]", "ExecuteFVGStrategy");
            }
            return;
         }
...
//                                                                        
// V29.00: 8AM CANDLE OPENING RANGE BREAKOUT (ORB)
// First H4 candle of London session defines the range
// Enter on midpoint retest in direction of breakout
//                                                                        
struct ORBState {
   double rangeHigh;
   double rangeLow;
   ...
   bool valid;
   datetime rangeTime;
   int breakoutDirection; // +1 = broke up, -1 = broke down, 0 = no breakout yet
};

ORBState g_orbState;
datetime g_lastORBScanTime = 0;

void UpdateORBState()
{
   if(Time[0] == g_lastORBScanTime) return;
   g_lastORBScanTime = Time[0];
   
   MqlDateTime dt;
   TimeToStruct(Time[1], dt);
   
   // Check if the completed bar is the range candle (first bar of London session)
   if(dt.hour == InpORB_RangeStartHour)
   {
      g_orbState.rangeHigh = High[1];
      g_orbState.rangeLow = Low[1];
      g_orbState.midpoint = (High[1] + Low[1]) / 2.0;
      g_orbState.valid = true;
      g_orbState.rangeTime = Time[1];
      g_orbState.breakoutDirection = 0;
      
      LogError(ERROR_INFO, "ORB Range Set: H=" + DoubleToString(g_orbState.rangeHigh,5) +
               " L=" + DoubleToString(g_orbState.rangeLow,5) +
               " Mid=" + DoubleToString(g_orbState.midpoint,5), "UpdateORBState");
   }
   
   // Check for breakout direction (after range is set)
   if(g_orbState.valid && g_orbState.breakoutDirection == 0)
   {
      TimeToStruct(TimeCurrent(), dt);
      if(dt.hour >= InpORB_EntryStartHour)
      {
         if(Close[1] > g_orbState.rangeHigh) g_orbState.breakoutDirection = +1;
         else if(Close[1] < g_orbState.rangeLow) g_orbState.breakoutDirection = -1;
      }
   }
}

void ExecuteORBStrategy()
{
   if(!InpORB_Enabled) return;
   if(Period() != PERIOD_H4) return;
   if(CountOpenTrades(InpORB_MagicNumber) >= InpORB_MaxConcurrent) return;
   if(!IsStrategyHealthy(InpORB_MagicNumber)) return;
   ...
   if(!g_orbState.valid) return;
   if(g_orbState.breakoutDirection == 0) return;
   
   // Check entry window
   MqlDateTime dt;
   TimeToStruct(TimeCurrent(), dt);
   if(dt.hour < InpORB_EntryStartHour || dt.hour >= InpORB_EntryEndHour) return;
   
   int stratIdx = GetStrategyIndex(InpORB_MagicNumber);
   double entryLevel = InpORB_UseMidpoint ? g_orbState.midpoint : 
                       (g_orbState.breakoutDirection == 1 ? g_orbState.rangeHigh : g_orbState.rangeLow);
   
   double slPips = InpORB_SL_Pips * Point * 10;
   double tpPips = InpORB_TP_Pips * Point * 10;
   
   // BUY: Breakout was upward, price retests midpoint from above
   if(g_orbState.breakoutDirection == 1)
   {
      if(Low[0] <= entryLevel && Close[0] > entryLevel)
      {
         if(InpORB_RequireEMAFilter)
         {
            double ema = iMA(Symbol(), PERIOD_H4, InpORB_EMA_Period, 0, MODE_EMA, PRICE_CLOSE, 0);
            if(Ask < ema) return;
         }
         ...
         double lots = MoneyManagement_Quantum(InpORB_MagicNumber, InpBase_Risk_Percent);
         if(lots <= 0) return;
         
         int ticket = OpenTrade(OP_BUY, lots, Ask, slPrice, tpPrice,
                                "ORB_BUY", InpORB_MagicNumber);
         if(ticket > 0)
         {
            g_orbState.valid = false; // Consume for today
            if(stratIdx >= 0) g_perfData[stratIdx].trades++;
            LogError(ERROR_INFO, "ORB BUY: Mid=" + DoubleToString(entryLevel,5), "ExecuteORBStrategy");
         }
         return;
      }
   }
   // SELL: Breakout was downward, price retests midpoint from below
   else if(g_orbState.breakoutDirection == -1)
   {
      if(High[0] >= entryLevel && Close[0] < entryLevel)
      {
         if(InpORB_RequireEMAFilter)
         {
            double ema = iMA(Symbol(), PERIOD_H4, InpORB_EMA_Period, 0, MODE_EMA, PRICE_CLOSE, 0);
         ...
         int ticket = OpenTrade(OP_SELL, lots, Bid, slPrice, tpPrice,
                                "ORB_SELL", InpORB_MagicNumber);
         if(ticket > 0)
         {
            g_orbState.valid = false;
            if(stratIdx >= 0) g_perfData[stratIdx].trades++;
            LogError(ERROR_INFO, "ORB SELL: Mid=" + DoubleToString(entryLevel,5), "ExecuteORBStrategy");
         }
         return;
      }
   }
...
void OnNewBar()
{
   ...
   // V29.00: Multi-Timeframe Trend Alignment filter
   // Blocks all new entries if TFs are not aligned
   int g_mtfBias = -1; // Global for this bar: -1=blocked, OP_BUY, OP_SELL
   if(InpMTF_Enabled)
   {
      g_mtfBias = GetMultiTimeframeBias();
      if(g_mtfBias == -1)
      {
         LogError(ERROR_INFO, "OnNewBar: MTF filter   no alignment, blocking entries", "OnNewBar");
         // Don't return   still run trailing stops and management. Just skip new entries below.
      }
   }
   
   // V29.00: Session Kill Zone filter
   if(InpKillZone_Enabled && !IsInKillZone())
   {
      LogError(ERROR_INFO, "OnNewBar: Outside Kill Zone, blocking entries", "OnNewBar");
      if(!IsOptimization()) UpdateDashboard_StaticV8_6();
      // Don't return   trailing stops still active. Block entries below.
      // We set mtfBias to -1 to block all entries
      g_mtfBias = -1;
   }
...
// }
   ...
   // V27 NOISE BREAKOUT STRATEGY: BB Squeeze + Breakout (SSRN-4824172)
   ...
   // V29.00: New strategy dispatch   only if MTF/KillZone allow
   if(g_mtfBias != -1 || !InpMTF_Enabled)
   {
      // V29.00   ICT Order Block
      if(InpOrderBlock_Enabled)
      {
         ExecuteOrderBlockStrategy();
         if(CountOpenTrades() >= InpMaxOpenTrades) { if(!IsOptimization()) UpdateDashboard_StaticV8_6(); return; }
      }
      
      // V29.00   Fair Value Gap (FVG + Liquidity Sweep)
      if(InpFVG_Enabled)
      {
         ExecuteFVGStrategy();
         if(CountOpenTrades() >= InpMaxOpenTrades) { if(!IsOptimization()) UpdateDashboard_StaticV8_6(); return; }
...
bool IsOurMagicNumber(int magic)
{
    if(magic == InpMagic_MeanReversion || 
       ...
       magic == InpOrderBlock_MagicNumber ||             // V29.00: ICT Order Block
       magic == InpFVG_MagicNumber ||                    // V29.00: FVG
       magic == InpORB_MagicNumber)                      // V29.00: ORB
    {
        return true;
    }
    return false;
...
int GetStrategyIndexFromMagic(int magicNumber) 
{
    ...
    if(magicNumber == InpOrderBlock_MagicNumber) return 17;       // V29.00: ICT Order Block
    if(magicNumber == InpFVG_MagicNumber) return 18;              // V29.00: FVG
    if(magicNumber == InpORB_MagicNumber) return 19;              // V29.00: ORB
...
void ReconcileFinalPerformance()
{
   ...
   PerfData reconciledData[20]; // V29.00: Extended to 20 strategies
...
int GetStrategyIndex(int magic)
{
   ...
   if(magic == InpOrderBlock_MagicNumber) return 17;       // V29.00: ICT Order Block
   if(magic == InpFVG_MagicNumber) return 18;              // V29.00: FVG
   if(magic == InpORB_MagicNumber) return 19;              // V29.00: ORB
   return -1;
}
...
void ExecuteApexStrategy()
{
   ...
   if(Close[1] > Open[1] + trigger)
   {
      ...
      if(tp < Bid - 5 * Point) // Sanity
      {
         ...
         if(ticket > 0 && stratIdx >= 0 && stratIdx < 20)
...
void MonitorPerformanceTargets() {
    ...
    if(systemPF >= 2.5 && systemWinRate >= 60 && tradesPerDay >= 15 && g_current_drawdown < 10) {
        ...
        if(systemPF < 2.5) LogError(ERROR_WARNING, "WARNING:   PF below 2.5 target - triggering ultra-aggressive optimization", "MonitorPerformanceTargets");
        if(systemWinRate < 60) LogError(ERROR_WARNING, "WARNING:   Win rate below 60% - adjusting entry criteria", "MonitorPerformanceTargets");  
        if(tradesPerDay < 15) LogError(ERROR_WARNING, "WARNING:   Trade frequency below 15/day - reducing filtering thresholds", "MonitorPerformanceTargets");
        if(g_current_drawdown >= 10) LogError(ERROR_WARNING, "WARNING:   Drawdown above 10% - activating defensive protocols", "MonitorPerformanceTargets");
    ...
    for(int i = 0; i < 7; i++) {
        if(g_perfData[i].trades > 0) {
            ...
            if(g_strategyCooldown[i].disabled) {
                LogError(ERROR_WARNING, "WARNING:   " + g_perfData[i].name + " in 10-bar cooldown period", "MonitorPerformanceTargets");
            }
        }
    }
}
...
double MoneyManagement_Quantum(int magicNumber, double baseRiskPercent, double stopLossPips = 0)
{
   ...
   if(idx >= 0 && idx < 20 && g_stratTotalTrades[idx] >= 10)
   {
   ...
   else
   {
      ...
      else if (magicNumber == InpOrderBlock_MagicNumber) maxMultiplier = 1.0; // V29.00: ICT Order Block   unproven
      else if (magicNumber == InpFVG_MagicNumber) maxMultiplier = 1.0;        // V29.00: FVG   unproven
      else if (magicNumber == InpORB_MagicNumber) maxMultiplier = 1.0;        // V29.00: ORB   unproven
   }
   
   //     ADAPTIVE RISK UNWIND (V27.8 legacy, still active)    
   double adaptiveMultiplier = 1.0;
   if(idx >= 0 && idx < 20) adaptiveMultiplier = MathMin(g_strategyMultiplier[idx], maxMultiplier);
   ...
   if(idx >= 0 && idx < 20 && g_stratTotalTrades[idx] >= 15)
   {
...
{
   // V29.00 FIX: Unified mapping   must match GetStrategyIndexFromMagic exactly
   // This was the V28.09 catastrophe source (wrong indices for MoneyManagement)
   if(magicNumber == InpMagic_MeanReversion || magicNumber == 555001) return 0;
   // Index 1 (Quantum Oscillator) is disabled
   if(magicNumber == InpTitan_MagicNumber) return 2;
   if(magicNumber == InpWarden_MagicNumber || magicNumber == 666001 || magicNumber == 666002) return 3;
   if(magicNumber == InpReaper_BuyMagicNumber || magicNumber == InpReaper_SellMagicNumber) return 4;
   if(magicNumber == InpSX_MagicNumber) return 5;
   if(magicNumber == InpChronos_MagicNumber) return 6;
   if(magicNumber == 777012) return 7; // NoiseBreakout
   if(magicNumber == 999002) return 11; // MathReversal
   if(magicNumber == InpApex_MagicNumber) return 8;
   if(magicNumber == InpPhantom_MagicNumber) return 9;
   if(magicNumber == InpNexus_MagicNumber) return 10;
   if(magicNumber == InpVortex_MagicNumber) return 11;
   if(magicNumber == InpRegimeShift_MagicNumber) return 12;
   if(magicNumber == InpSessionMomentum_MagicNumber) return 13;
   if(magicNumber == InpDivergenceMR_MagicNumber) return 14;
   if(magicNumber == InpLiquiditySweep_MagicNumber) return 15;
   if(magicNumber == InpStructuralRetest_MagicNumber) return 16;
   if(magicNumber == InpOrderBlock_MagicNumber) return 17;
   if(magicNumber == InpFVG_MagicNumber) return 18;
   if(magicNumber == InpORB_MagicNumber) return 19;
   return -1;
}
