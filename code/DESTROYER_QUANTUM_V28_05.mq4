...
//|  DESTROYER QUANTUM V28.05 - VENI VIDI VICI                        |
...
bool IsHighConvictionSignal(int order_type)
{
    ...
    // FINAL DECISION  3-of-3 layers required for A+ setups only
    if(confluenceCount >= 3) {
        LogError(ERROR_INFO, "Reaper Elite Signal: 3-of-3 confluence achieved (ALL layers confirmed).");
        return true;
    }
...
if(!InpDisable_VAR_Limiter && riskAmount > m_portfolioVAR * 0.5) {
  // V28.05 FIX #1: Relaxed VAR threshold from 0.1 to 0.5
  // Old 0.1 was blocking nearly everything in low-vol conditions
  LogError(ERROR_WARNING, "Risk Manager: Trade exceeds portfolio VAR limit (threshold 0.5)", "ApproveTrade");
  return false;
}
