...
//|  DESTROYER QUANTUM V28.10B - VENI VIDI VICI                       |
...
==================================================================================================================
   ...
   *** V27.19 FEATURES:
   ...
   2 DYNAMIC TIER CAPS (Performance-Based, Not Hardcoded)
      ...
      - New: PF3.0 -> 4.0x, PF2.0 -> 3.0x, PF1.5 -> 2.5x, PF1.2 -> 2.0x, PF<1.0 -> 0.75x
...
StrategyCooldown g_strategyCooldown[18]; // V28.00: Extended to 17 strategies
// ---
...
double GetStrategySpecificRisk(int magicNumber)
{
   ...
   // 5. SENTINEL (V28.10B: Conservative - new strategy)
   if(magicNumber == InpSentinel_MagicNumber) // 777015
      return 0.8; // Conservative 0.8x until proven
...
// V28.10B: SENTINEL - EMA Crossover Trend Follower
sinput string InpSentinel_Header = "====== SENTINEL: EMA CROSSOVER ======";
input bool    InpSentinel_Enabled       = false;      // Disabled - needs rework (PF 0.55)
input int     InpSentinel_MagicNumber   = 777015;
input int     InpSentinel_FastEMA       = 21;
input int     InpSentinel_SlowEMA       = 50;
input int     InpSentinel_RSI_Period    = 14;
input double  InpSentinel_RSI_Buy       = 40.0;
input double  InpSentinel_RSI_Sell      = 60.0;
input double  InpSentinel_ATR_SL_Mult   = 1.5;
input double  InpSentinel_RR            = 2.0;
input int     InpSentinel_MaxOpen       = 1;
...
int OnInit()
{
   ...
   for(int i=0; i<18; i++) // V28.00: Extended to 17 strategy slots
   {
   ...
   g_perfData[9].name = "Phantom";
    ...
    g_perfData[17].name = "Sentinel"; // V28.10B: New strategy
   // ---
...
// }
   ...
   // V28.10B: SENTINEL - EMA Crossover (disabled by default)
...
if(magicNumber == InpStructuralRetest_MagicNumber) return 16; // V28.03: Structural Retest
    if(magicNumber == InpSentinel_MagicNumber) return 17; // V28.10B: Sentinel
...
void ReconcileFinalPerformance()
{
   ...
   PerfData reconciledData[18]; // V28.00: Extended to 17 strategies
   for(int i=0; i<18; i++) // V28.00: Extended to 17 strategies
   {
...
int GetStrategyIndex(int magic)
{
   ...
   if(magic == InpSentinel_MagicNumber) return 17; // V28.10B: Sentinel
   return -1;
}
...
void ExecuteSentinel()
{
   ...
   {
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
         if(OrderMagicNumber() == InpSentinel_MagicNumber) sentinelCount++;
   }
   if(sentinelCount >= InpSentinel_MaxOpen) return;
   
   double fastEMA = iMA(Symbol(), PERIOD_H4, InpSentinel_FastEMA, 0, MODE_EMA, PRICE_CLOSE, 1);
   double slowEMA = iMA(Symbol(), PERIOD_H4, InpSentinel_SlowEMA, 0, MODE_EMA, PRICE_CLOSE, 1);
   double rsi = iRSI(Symbol(), PERIOD_H4, InpSentinel_RSI_Period, PRICE_CLOSE, 1);
   double atr = iATR(Symbol(), PERIOD_H4, 14, 1);
   
   if(atr <= 0 || fastEMA <= 0 || slowEMA <= 0) return;
   
   double slDist = atr * InpSentinel_ATR_SL_Mult;
   double tpDist = slDist * InpSentinel_RR;
   
   if(fastEMA > slowEMA && rsi > InpSentinel_RSI_Buy && rsi < 70)
   {
      ...
      if(lots > 0)
      {
         int ticket = OpenTrade(OP_BUY, lots, Ask, sl, tp, "SENTINEL_BUY", InpSentinel_MagicNumber);
         if(ticket > 0)
         {
   ...
   else if(fastEMA < slowEMA && rsi < InpSentinel_RSI_Sell && rsi > 30)
   {
      ...
      if(lots > 0)
      {
         int ticket = OpenTrade(OP_SELL, lots, Bid, sl, tp, "SENTINEL_SELL", InpSentinel_MagicNumber);
         if(ticket > 0)
         {
            int stratIdx = GetStrategyIndex(InpSentinel_MagicNumber);
            if(stratIdx >= 0) g_perfData[stratIdx].trades++;
...
double MoneyManagement_Quantum(int magicNumber, double baseRiskPercent, double stopLossPips = 0)
{
   ...
   else
   {
      ...
      else if (magicNumber == InpSentinel_MagicNumber) maxMultiplier = 1.0; // V28.10B: Sentinel - unproven
   }
