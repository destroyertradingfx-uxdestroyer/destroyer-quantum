...
//|  DESTROYER QUANTUM V28.07 - VENI VIDI VICI                        |
...
extern double InpReaper_LotMultiplier = 1.15;   // V28.07: Reduced from 1.3  cuts deep-level risk by 58%
...
extern double InpReaper_TrailStart_Money = 300.0;  // V28.07: Raised from 150  trail at 75% of base TP.
...
void ProcessReaperBasket(int magic_number, int order_type)
{
   ...
   for(int i = 0; i < OrdersTotal(); i++)
   {
    ...
    // --- BASKET TP CHECK ---
    // V28.07 FIX #3: DYNAMIC BASKET TP by grid depth
    // Deeper grids = bigger move captured = bigger reward
    double dynamicTP = InpReaper_BasketTP_Money; // Default: $400
    if(basket_levels <= 3)      dynamicTP = 250.0;  // Shallow grid: quick scalp
    else if(basket_levels <= 5) dynamicTP = 400.0;  // Standard depth
    else                        dynamicTP = 600.0;  // Deep grid: maximum capture
    
    if(basket_profit >= dynamicTP)
   {
      CloseAllByMagic(magic_number);
      LogError(ERROR_INFO, "Reaper Basket CLOSED - Dynamic TP $" + DoubleToString(dynamicTP, 0) + 
                " reached! Profit: $" + DoubleToString(basket_profit, 2) + 
                " (levels: " + IntegerToString(basket_levels) + ")", "ProcessReaperBasket");
      return;
   }
   ...
   // 
   // V28.07 FIX #1: DYNAMIC GRID LEVELS BY ADX
   // Full grid in calm waters, surgical in storms
   // 
   double adx_for_cap = iADX(Symbol(), PERIOD_H4, 14, PRICE_CLOSE, MODE_MAIN, 1);
   int dynamicMaxLevels = InpReaper_HardcapLevels; // Default: 8
   if(adx_for_cap >= 40.0)      dynamicMaxLevels = 4;  // Strong trend: 4 levels max
   else if(adx_for_cap >= 25.0) dynamicMaxLevels = 6;  // Moderate trend: 6 levels
   
   if(basket_levels >= dynamicMaxLevels)
   {
      LogError(ERROR_WARNING, "Reaper DYNAMIC HARDCAP: ADX=" + DoubleToString(adx_for_cap,1) + 
                " allows " + IntegerToString(dynamicMaxLevels) + " levels (at " + 
                IntegerToString(basket_levels) + ") - No more entries", "ProcessReaperBasket");
      return;
   }
