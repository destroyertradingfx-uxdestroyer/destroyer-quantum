...
void ManageOpenTradesV13_ELITE()
{
    ...
    for(int i = OrdersTotal() - 1; i >= 0; i--)
    {
       ...
       if(OrderType() == OP_SELL && profitR > 0)
       {
        ...
        // V28.15: STAGE 1 — Secure at 1.5R (was 1.0R — too aggressive, killing winners)
        if (profitR >= 1.5 && !isAtBreakEven)
        {
...
public:
    ...
    void ApplyEliteTrailingStop(int ticket) {
        ...
        // V28.15: BREAK-EVEN AT 1.5R (was 1.0R — too aggressive, killing winners)
        if(profitR >= 1.5 && OrderStopLoss() < openPrice) {
