...
double         g_pipValue = 0;                   // Initialized in InitDebateLayer
...
void InitDebateLayer() {
   ...
   if(g_pipValue <= 0) g_pipValue = 0.0001;  // Fallback
   
   LogError(ERROR_INFO, "DEBATE LAYER: Initialized. " + IntegerToString(MAX_SIGNALS) + " signal slots.");
}
...
bool SubmitSignal(int magic, int direction, double conviction,
   ...
   if (g_signalCount >= MAX_SIGNALS) {
      LogError(ERROR_INFO, "DEBATE: Signal buffer full. Dropping signal from magic " + IntegerToString(magic));
      return false;
   }
...
DebateResult RunDebate() {
   ...
   if (result.buyConviction > result.sellConviction) {
      ...
      //--- Exact tie -- no trade
      LogError(ERROR_INFO, "DEBATE: Exact tie between BUY and SELL. No trade.");
      return result;
   }
   ...
   if (result.isDivergent) {
      LogError(ERROR_INFO, "DEBATE: DIVERGENCE detected. BUY=" + DoubleToStr(result.buyConviction, 2) +
   ...
   if (winningConviction < MIN_CONVICTION_THRESHOLD) {
      LogError(ERROR_INFO, "DEBATE: Conviction " + DoubleToStr(winningConviction, 2) +
   ...
   LogError(ERROR_INFO, "DEBATE: APPROVED " + (result.direction == OP_BUY ? "BUY" : "SELL") +
...
bool ConservativeRiskCheck(StrategySignal &signal, string &reason) {
   ...
   if (g_consecutiveLosses >= CONSERVATIVE_MAX_CONSEC_LOSSES) {
      reason = "Consecutive losses: " + IntegerToString(g_consecutiveLosses) +
...
RiskPanelResult RunRiskPanel(StrategySignal &signal) {
   ...
   LogError(ERROR_INFO, "RISK PANEL: " + IntegerToString(result.approvals) + "/3 approved" +
   ...
   if (result.approvals < RISK_PANEL_MIN_APPROVALS) {
      if (!result.conservativeApproved)
         LogError(ERROR_INFO, "RISK PANEL REJECT (conservative): " + result.conservativeReason);
      if (!result.neutralApproved)
         LogError(ERROR_INFO, "RISK PANEL REJECT (neutral): " + result.neutralReason);
   }
...
double GetDebateSizeMultiplier(double conviction, int approvals, bool isDivergent) {
   ...
   if (isDivergent && baseMultiplier > 0) {
      baseMultiplier *= 0.5;
      LogError(ERROR_INFO, "SIZING: Divergence penalty applied. Size reduced 50%.");
   }
...
void ExecuteNexusStrategy_DEBATE()
{
   ...
   if(compressionBars < InpNexus_CompressionBars) return;
...
//+------------------------------------------------------------------+
//| HELPER: GetStrategyNameByMagic                                    |
//+------------------------------------------------------------------+
string GetStrategyNameByMagic(int magic)
{
   if(magic == InpMagic_MeanReversion) return "MeanReversion";
   if(magic == 999002) return "MathReversal";
   if(magic == InpNoiseBreakout_Magic) return "NoiseBreakout";
   if(magic == InpApex_MagicNumber) return "Apex";
   if(magic == InpNexus_MagicNumber) return "Nexus";
   if(magic == InpChronos_MagicNumber) return "Microstructure";
   if(magic == InpVortex_MagicNumber) return "Vortex";
   if(magic == InpRegimeShift_MagicNumber) return "RegimeShift";
   if(magic == InpSessionMomentum_MagicNumber) return "SessionMomentum";
   if(magic == InpDivergenceMR_MagicNumber) return "DivergenceMR";
   if(magic == InpStructuralRetest_MagicNumber) return "StructuralRetest";
   if(magic == InpPhantom_MagicNumber) return "Phantom";
   if(magic == InpReaper_BuyMagicNumber) return "Reaper_BUY";
   if(magic == InpReaper_SellMagicNumber) return "Reaper_SELL";
   return "Unknown_" + IntegerToString(magic);
}

//+------------------------------------------------------------------+
//| HELPER: GetTotalExposurePercent                                   |
//+------------------------------------------------------------------+
double GetTotalExposurePercent()
{
   ...
   return (totalLots / maxLots) * 100.0;
}

//+------------------------------------------------------------------+
//| HELPER: CountTradesByDirection                                    |
//+------------------------------------------------------------------+
int CountTradesByDirection(int direction)
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol()) continue;
      if(direction == OP_BUY && OrderType() == OP_BUY) count++;
      if(direction == OP_SELL && OrderType() == OP_SELL) count++;
   }
   return count;
}

//+------------------------------------------------------------------+
//| HELPER: HasOpenPositionByMagic                                    |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| FUNCTION: ExecuteSessionMomentum_DEBATE                           |
//| Session Momentum signal submission (no direct execution)          |
//+------------------------------------------------------------------+
void ExecuteSessionMomentum_DEBATE()
{
   ...
   double adxNorm = MathMin(1.0, (adx - InpSessionMomentum_ADX_Threshold) / 20.0);
   double rangeRatio = londonRange / atr;
   double conviction = MathMin(1.0, adxNorm * 0.5 + MathMin(1.0, rangeRatio / 3.0) * 0.5);
   ...
   if(buySignal && (bias == 1 || bias == 2))
   {
      ...
      if(lots > 0)
         SubmitSignal(InpSessionMomentum_MagicNumber, OP_BUY, conviction,
                      "SESSION_MOM|ADX=" + DoubleToStr(adx, 1), lots, sl, tp);
   }
   else if(sellSignal && (bias == -1 || bias == 2))
   {
      ...
      double lots = MoneyManagement_Quantum(InpSessionMomentum_MagicNumber, InpBase_Risk_Percent);
      if(lots > 0)
         SubmitSignal(InpSessionMomentum_MagicNumber, OP_SELL, conviction,
                      "SESSION_MOM|ADX=" + DoubleToStr(adx, 1), lots, sl, tp);
   }
}

//+------------------------------------------------------------------+
//| FUNCTION: ExecuteDivergenceMR_DEBATE                              |
//| Divergence MR signal submission (no direct execution)             |
//+------------------------------------------------------------------+
void ExecuteDivergenceMR_DEBATE()
{
   ...
   double rsiDev = MathAbs(rsi_1 - 50.0) / 50.0;
   double bbDist = (bbRange > 0) ? MathAbs(Close[1] - (bbUpper + bbLower) / 2.0) / (bbRange / 2.0) : 0;
   double conviction = MathMin(1.0, rsiDev * 0.4 + bbDist * 0.3 + (1.0 - hurst) * 0.3);
   ...
   if(bullishDivergence && atBBLower && (bias == 1 || bias == 2))
   {
      ...
      if(lots > 0)
         SubmitSignal(InpDivergenceMR_MagicNumber, OP_BUY, conviction,
                      "DIV_MR|H=" + DoubleToStr(hurst, 2) + " RSI=" + DoubleToStr(rsi_1, 1), lots, sl, tp);
   }
   else if(bearishDivergence && atBBUpper && (bias == -1 || bias == 2))
   {
      ...
      double lots = MoneyManagement_Quantum(InpDivergenceMR_MagicNumber, InpBase_Risk_Percent);
      if(lots > 0)
         SubmitSignal(InpDivergenceMR_MagicNumber, OP_SELL, conviction,
                      "DIV_MR|H=" + DoubleToStr(hurst, 2) + " RSI=" + DoubleToStr(rsi_1, 1), lots, sl, tp);
   }
}

//+------------------------------------------------------------------+
//| FUNCTION: ExecuteStructuralRetest_DEBATE                          |
//| Structural Retest signal submission (no direct execution)         |
//+------------------------------------------------------------------+
void ExecuteStructuralRetest_DEBATE()
{
   ...
   if(brokeAbove && breakBar > 1)
   {
      ...
      if(retesting && (bias == 1 || bias == 2))
      {
         ...
         {
            double conviction = MathMin(1.0, rr / 4.0 + (double)breakBar / (double)InpStructuralRetest_RetraceBars * 0.3);
            double lots = MoneyManagement_Quantum(InpStructuralRetest_MagicNumber, InpBase_Risk_Percent);
            if(lots > 0)
               SubmitSignal(InpStructuralRetest_MagicNumber, OP_BUY, conviction,
                            "RETEST_BUY|RR=" + DoubleToStr(rr, 1), lots, sl, tp);
         }
      }
   }
   ...
   if(brokeBelow && breakBar > 1)
   {
      ...
      if(retesting && (bias == -1 || bias == 2))
      {
         ...
         if(rr >= InpStructuralRetest_MinRR)
         {
            ...
            double lots = MoneyManagement_Quantum(InpStructuralRetest_MagicNumber, InpBase_Risk_Percent);
            if(lots > 0)
               SubmitSignal(InpStructuralRetest_MagicNumber, OP_SELL, conviction,
                            "RETEST_SELL|RR=" + DoubleToStr(rr, 1), lots, sl, tp);
         }
      }
   }
}
...
//+------------------------------------------------------------------+
//| FUNCTION: ExecutePhantomStrategy_DEBATE                           |
//| Phantom Monday signal submission (no direct execution)            |
//+------------------------------------------------------------------+
void ExecutePhantomStrategy_DEBATE()
{
   ...
   // Phantom: Monday gap fade - enter opposite to weekend gap
   double fridayClose = iClose(Symbol(), PERIOD_H4, iBarShift(Symbol(), PERIOD_H4, TimeCurrent() - 86400 * 3));
   double mondayOpen = Open[iBarShift(Symbol(), PERIOD_H4, TimeCurrent())];
   double gap = mondayOpen - fridayClose;
   double gapPips = MathAbs(gap) / g_pipValue;

   if(gapPips < 5) return; // Minimum gap size

   double rsi = iRSI(Symbol(), PERIOD_H4, 14, PRICE_CLOSE, 1);
   double conviction = MathMin(1.0, gapPips / 30.0 * 0.6 + MathAbs(rsi - 50.0) / 50.0 * 0.4);

   // Use gap-based SL/TP (consistent with Phantom design)
   double gapDistance = MathAbs(gap);
   double slDist = gapDistance * InpPhantom_SL_GapMult;
   double tpDist = gapDistance * InpPhantom_TP_GapMult;

   if(gap > 0 && (bias == -1 || bias == 2)) // Gap up, sell the fade
   {
      double sl = Ask + slDist;
      double tp = Bid - tpDist;
      double lots = MoneyManagement_Quantum(InpPhantom_MagicNumber, InpBase_Risk_Percent);
      if(lots > 0)
         SubmitSignal(InpPhantom_MagicNumber, OP_SELL, conviction,
                      "PHANTOM|GAP=" + DoubleToStr(gapPips, 0), lots, sl, tp);
   }
   else if(gap < 0 && (bias == 1 || bias == 2)) // Gap down, buy the fade
   {
      double sl = Bid - slDist;
      double tp = Ask + tpDist;
      double lots = MoneyManagement_Quantum(InpPhantom_MagicNumber, InpBase_Risk_Percent);
      if(lots > 0)
         SubmitSignal(InpPhantom_MagicNumber, OP_BUY, conviction,
                      "PHANTOM|GAP=" + DoubleToStr(gapPips, 0), lots, sl, tp);
   }
}
...
//+------------------------------------------------------------------+
//| FUNCTION: ExecuteDebateTrade                                      |
//| Runs debate: weighted voting + risk panel + execute winner        |
//| Returns ticket number or -1 if no trade                          |
//+------------------------------------------------------------------+
int ExecuteDebateTrade()
{
   if(g_signalCount == 0) return -1;

   // Step 1: Weighted voting
   double buyConviction = 0;
   double sellConviction = 0;
   string buyStrategies = "";
   string sellStrategies = "";

   for(int i = 0; i < g_signalCount; i++)
   {
      double weight = GetStrategyWeight(g_signals[i].magic);
      double weightedConv = g_signals[i].conviction * weight;
      string name = GetStrategyNameByMagic(g_signals[i].magic);

      if(g_signals[i].direction == OP_BUY)
      {
         buyConviction += weightedConv;
         if(buyStrategies != "") buyStrategies += ",";
         buyStrategies += name;
      }
      else if(g_signals[i].direction == OP_SELL)
      {
         sellConviction += weightedConv;
         if(sellStrategies != "") sellStrategies += ",";
         sellStrategies += name;
      }
   }

   // Step 2: Determine winner
   int winnerDirection = -1;
   double winningConviction = 0;

   if(MathAbs(buyConviction - sellConviction) < 0.05)
   {
      // Tie - no trade
      LogError(ERROR_INFO, "DEBATE: Exact tie between BUY and SELL. No trade.", "ExecuteDebateTrade");
      return -1;
   }

   if(buyConviction > sellConviction)
   {
      if(buyConviction < MIN_CONVICTION_THRESHOLD)
      {
         LogError(ERROR_INFO, "DEBATE: BUY conviction " + DoubleToStr(buyConviction, 2) + " below threshold " + DoubleToStr(MIN_CONVICTION_THRESHOLD, 2), "ExecuteDebateTrade");
         return -1;
      }
      winnerDirection = OP_BUY;
      winningConviction = buyConviction;
   }
   else
   {
      if(sellConviction < MIN_CONVICTION_THRESHOLD)
      {
         LogError(ERROR_INFO, "DEBATE: SELL conviction " + DoubleToStr(sellConviction, 2) + " below threshold " + DoubleToStr(MIN_CONVICTION_THRESHOLD, 2), "ExecuteDebateTrade");
         return -1;
      }
      winnerDirection = OP_SELL;
      winningConviction = sellConviction;
   }

   // Check for divergence (conflicting signals)
   bool isDivergent = (buyConviction > DIVERGENCE_THRESHOLD && sellConviction > DIVERGENCE_THRESHOLD);

   // Step 3: Find best signal in winning direction
   StrategySignal bestSignal;
   bestSignal.direction = -1;
   double bestWeightedConv = 0;

   for(int j = 0; j < g_signalCount; j++)
   {
      if(g_signals[j].direction == winnerDirection)
      {
         double w = GetStrategyWeight(g_signals[j].magic) * g_signals[j].conviction;
         if(w > bestWeightedConv)
         {
            bestWeightedConv = w;
            bestSignal = g_signals[j];
         }
      }
   }
   ...
   double finalLots = bestSignal.suggestedLots * sizeMult;
   finalLots = MathMax(finalLots, MarketInfo(Symbol(), MODE_MINLOT));
   finalLots = MathMin(finalLots, MarketInfo(Symbol(), MODE_MAXLOT));

   // Step 6: Execute
   int ticket = OpenTrade(winnerDirection, finalLots,
                          (winnerDirection == OP_BUY) ? Ask : Bid,
                          bestSignal.suggestedSL, bestSignal.suggestedTP,
                          bestSignal.reason, bestSignal.magic);

   if(ticket > 0)
   {
      LogError(ERROR_INFO, "DEBATE: APPROVED " + (winnerDirection == OP_BUY ? "BUY" : "SELL") +
               " conviction=" + DoubleToStr(winningConviction, 2) +
               " size=" + DoubleToStr(finalLots, 2) +
               " strategies=" + (winnerDirection == OP_BUY ? buyStrategies : sellStrategies), "ExecuteDebateTrade");

      // Update performance tracking
      int stratIdx = GetStrategyIndex(bestSignal.magic);
      if(stratIdx >= 0) g_perfData[stratIdx].trades++;

      // Log to debate trade log
      if(g_tradeLogCount < 100)
      {
         g_tradeLog[g_tradeLogCount].ticket = ticket;
         g_tradeLog[g_tradeLogCount].direction = winnerDirection;
         g_tradeLog[g_tradeLogCount].entryPrice = (winnerDirection == OP_BUY) ? Ask : Bid;
         g_tradeLog[g_tradeLogCount].lots = finalLots;
         g_tradeLog[g_tradeLogCount].conviction = winningConviction;
         g_tradeLog[g_tradeLogCount].riskApprovals = approvals;
         g_tradeLog[g_tradeLogCount].strategiesAgreed = (winnerDirection == OP_BUY) ? buyStrategies : sellStrategies;
         g_tradeLog[g_tradeLogCount].strategiesDisagreed = (winnerDirection == OP_BUY) ? sellStrategies : buyStrategies;
         g_tradeLog[g_tradeLogCount].atrAtEntry = iATR(Symbol(), PERIOD_H4, 14, 1);
         g_tradeLog[g_tradeLogCount].hurstAtEntry = CalculateHurstExponent(Symbol(), Period(), 100);
         g_tradeLog[g_tradeLogCount].adxAtEntry = iADX(Symbol(), PERIOD_H4, 14, PRICE_CLOSE, MODE_MAIN, 1);
         g_tradeLogCount++;
      }
   }
...
//| Call when a debate trade closes to log outcome + deferred reflect |
//+------------------------------------------------------------------+
void ProcessTradeClose(int ticket, double exitPrice, double pnl)
{
   for(int i = 0; i < g_tradeLogCount; i++)
   {
      if(g_tradeLog[i].ticket == ticket)
      {
         ...
         g_tradeLog[i].holdDuration = TimeCurrent() - TimeCurrent(); // Will be set by caller
         g_tradeLog[i].thesisCorrect = (pnl > 0);

         // Deferred reflection: adjust strategy weights based on outcome
         if(g_tradeLog[i].thesisCorrect)
         {
            // Winning trade: slight boost to participating strategies
            // No change needed  Kelly/Heat system handles this
         }
         else
         {
            // Losing trade: slight penalty if consensus was strong
            if(g_tradeLog[i].riskApprovals >= 3 && g_tradeLog[i].conviction > 0.7)
            {
               // High-confidence loss = market changed, be cautious
               LogError(ERROR_INFO, "REFLECTION: High-confidence loss detected. Monitoring for regime shift.", "ProcessTradeClose");
            }
         }
         break;
      }
...
void OnNewBar_DebateLayer()
{
   ...
   if(g_signalCount > 0) {
      ...
      if(ticket > 0) {
         LogError(ERROR_INFO, "ONNEWBAR_DEBATE: Trade executed #" + IntegerToString(ticket) +
