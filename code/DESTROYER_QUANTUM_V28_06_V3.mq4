//+------------------------------------------------------------------+
//|        DESTROYER_QUANTUM_V28_06_RESURRECTION.mq4                   |
//|                    Copyright 2026, Quantum Leap Analytics        |
//|  DESTROYER QUANTUM V28.06 RESURRECTION - VENI VIDI VICI           |
...
#property version   "28.06"  // V28.06 RESURRECTION: Aggressive defaults + Silicon-X revived + critical trail fixes
#property strict
...
extern bool    InpChronos_Enabled = false;              // V28.07: DISABLED - M15 HFT causes tester iteration overflow
...
extern int     InpMaxOpenTrades      = 24;          // V28.07 FIX: Raised from 16 ? Reaper alone uses 16 grid slots
...
extern bool    InpDisable_VAR_Limiter  = true;   // V28.07: DISABLED - was blocking 60%+ of trades  // V27.1 FIX: VAR gate MUST be active ? was bypassed in V27 causing runaway exposure
extern bool    InpDisable_AlphaSentinel = true;   // V28.07: DISABLED - was blocking all low-conviction entries // V27.1 FIX: Alpha Sentinel MUST be active ? was bypassed in V27 causing unfiltered basket initiations
...
extern bool    InpTradeWednesday       = true;       // V28.07: ENABLED - re-enabling for 20% more trading days       // V27.20 FIX: Block Wednesday trading (44% loss rate)
...
extern int    InpReaper_TrailStop_Pips    = 150;       // V28.07 FIX: Lowered from 400 ? was 3-5x ATR, trail never tightened
...
// ============================================================
extern bool    InpNexus_Enabled           = false;  // V28.07: KILLED - PF 0.37, -$9,525 net loss
...
int      g_consecLossTracker[18][2];                // [idx][0]=streak(-1=losses,1=wins) [idx][1]=count
datetime g_strategyLockoutUntil[18];                // Per-strategy lockout timestamps
...
double   g_strategyMultiplier[18];                 // Dynamic risk multipliers [0-17] per strategy index
...
int      g_stratProfitIdx[18];                     // Current circular buffer index per strategy
int      g_stratTotalTrades[18];                   // Total trades completed per strategy
double   g_stratRollingWinRate[18];                // Rolling win rate (EWMA)
double   g_stratRollingAvgWin[18];                 // Rolling average winning trade $
double   g_stratRollingAvgLoss[18];                // Rolling average losing trade $
double   g_stratRollingPF[18];                     // Rolling profit factor
double   g_stratKellyFraction[18];                 // Kelly-optimal fraction per strategy
...
StrategyCooldown g_strategyCooldown[18]; // V28.07: Extended to 18 strategies (Asian Breakout)
// ---
...
void UpdatePerformanceMetrics()
{
    ...
    if(total_trades > 10)
    {
        ...
        for(int i = 0; i < 18 && i < ArraySize(g_perfData); i++) // V28.07: Extended to 18
        {
            if(g_perfData[i].trades > 0)
            {
...
extern bool    InpVortex_Enabled         = true;       // V28.07: ENABLED ? proven trend-following, +$3K-$8K expected
...
extern bool    InpRegimeShift_Enabled         = true;  // V28.07: ENABLED ? ADX regime detection, +$2K-$5K expected
...
sinput string Inp_Header_AsianBreakout = "====== V28.07: ASIAN RANGE BREAKOUT (London Open) ======";
input bool    InpAsianBreakout_Enabled       = true;          // Enable Asian Range Breakout
input int     InpAsianBreakout_MagicNumber   = 9007;          // Magic number for Asian Breakout
input double  InpAsianBreakout_ATR_SL_Mult   = 1.5;           // ATR multiplier for SL
input double  InpAsianBreakout_ATR_TP_Mult   = 2.5;           // ATR multiplier for TP (1.67 R:R)
input double  InpAsianBreakout_RangeMinATR   = 0.3;           // Min Asian range as ATR multiple (Goldilocks low)
input double  InpAsianBreakout_RangeMaxATR   = 1.5;           // Max Asian range as ATR multiple (Goldilocks high)
input int     InpAsianBreakout_ADX_Period    = 14;             // ADX period for trend filter
input double  InpAsianBreakout_ADX_Threshold = 15.0;           // ADX threshold for trend confirmation

// V28.07: EFFICIENCY RATIO REGIME GATE
sinput string Inp_Header_ERGate = "====== V28.07: EFFICIENCY RATIO REGIME GATE ======";
sinput bool   InpERGate_Enabled     = true;       // Enable Efficiency Ratio regime gate
sinput int    InpERGate_Period      = 10;          // ER lookback period (bars)
sinput double InpERGate_Threshold   = 0.30;        // ER below this = choppy, reduce/block trend trades
sinput double InpERGate_LotScaleMin = 0.3;         // Min lot scale when ER is very low
sinput double InpERGate_LotScaleMax = 1.2;         // Max lot scale when ER is very high
...
// V28.07: Efficiency Ratio runtime state
double g_efficiencyRatio = 1.0;    // Current ER value (0=choppy, 1=perfect trend)
double g_erLotScale = 1.0;         // Lot size multiplier based on ER
...
int OnInit()
{
   ...
   g_perfData[9].name = "Phantom";
    ...
    g_perfData[17].name = "AsianBreakout";   // V28.07: Asian Range Breakout
   // ---
   
   // V13.0 ELITE: Initialize Strategy Cooldown System
   for(int i = 0; i < 18; i++)  // V28.07: Extended to 18 strategies
   {
...
//+------------------------------------------------------------------+
//| V28.07: EFFICIENCY RATIO REGIME GATE                             |
//| ER = |Close - Close[N]| / Sum(|Close[i] - Close[i-1]|, i=1..N)  |
//| Range: 0 (pure chop) to 1 (perfect straight-line trend)          |
//| Filters out choppy markets where trend strategies bleed           |
//+------------------------------------------------------------------+
void UpdateEfficiencyRatio()
{
   if(!InpERGate_Enabled) {
      g_efficiencyRatio = 1.0;
      g_erLotScale = 1.0;
      return;
   }
   
   int period = InpERGate_Period;
   ...
   // Efficiency Ratio
   g_efficiencyRatio = (volatility > 0) ? direction / volatility : 0;
   
   // Lot scaling: linear interpolation based on ER
   if(g_efficiencyRatio < InpERGate_Threshold) {
      // Choppy zone: scale from LotScaleMin at ER=0 to 1.0 at ER=threshold
      double ratio = g_efficiencyRatio / InpERGate_Threshold;
      g_erLotScale = InpERGate_LotScaleMin + ratio * (1.0 - InpERGate_LotScaleMin);
   } else {
      // Trending zone: scale from 1.0 at ER=threshold to LotScaleMax at ER=1.0
      double ratio = (g_efficiencyRatio - InpERGate_Threshold) / (1.0 - InpERGate_Threshold);
      g_erLotScale = 1.0 + ratio * (InpERGate_LotScaleMax - 1.0);
   }
   
   g_erLotScale = MathMax(InpERGate_LotScaleMin, MathMin(InpERGate_LotScaleMax, g_erLotScale));
}
...
void OnNewBar()
{
   ...
   // V28.07: Update Efficiency Ratio (regime gate)
   UpdateEfficiencyRatio();
   if(InpERGate_Enabled && g_efficiencyRatio < InpERGate_Threshold * 0.5) {
      // ER extremely low (< half threshold) = pure noise, block ALL entries
      LogError(ERROR_WARNING, "OnNewBar: ER " + DoubleToString(g_efficiencyRatio, 3) + 
         " < " + DoubleToString(InpERGate_Threshold * 0.5, 3) + " - PURE NOISE, blocking all entries", "OnNewBar");
      if(!IsOptimization()) UpdateDashboard_StaticV8_6();
      return;
   }
...
// }
   ...
   // V28.07 ? Asian Range Breakout (London Open)
   if(InpAsianBreakout_Enabled)
   {
      ExecuteAsianBreakoutStrategy();
      if(CountOpenTrades() >= InpMaxOpenTrades) { if(!IsOptimization()) UpdateDashboard_StaticV8_6(); return; }
   }
...
string GetStrategyNameFromMagic(int magic)
{
    ...
    if(magic == InpAsianBreakout_MagicNumber) return "AsianBreakout"; // V28.07
...
bool IsOurMagicNumber(int magic)
{
    if(magic == InpMagic_MeanReversion || 
       ...
       magic == InpAsianBreakout_MagicNumber)         // V28.07: Asian Range Breakout
    {
        return true;
    }
    return false;
...
int GetStrategyIndexFromMagic(int magicNumber) 
{
    ...
    if(magicNumber == 999002) return 7; // V28.07 FIX: MathReversal shares slot 7 with NoiseBreakout (was conflicting with Vortex at 11)
    ...
    if(magicNumber == InpAsianBreakout_MagicNumber) return 17; // V28.07: Asian Range Breakout
...
void ReconcileFinalPerformance()
{
   ...
   PerfData reconciledData[18]; // V28.07: Extended to 18 strategies
   for(int i=0; i<18; i++) // V28.07: Extended to 18 strategies
   {
...
int GetStrategyIndex(int magic)
{
   ...
   if(magic == InpAsianBreakout_MagicNumber) return 17; // V28.07: Asian Range Breakout
   return -1;
}
...
void ExecuteNexusStrategy()
{
   ...
   // V28.07 FIX: Use closed bar breakout (Close[1] > High[2]) instead of gap condition (Close[0] > High[1]).
   // OLD BUG: On new H4 bar, Close[0] = Open[0], so Close[0]>High[1] = gap condition (almost never on EURUSD).
   bool buySignal  = (Close[1] > High[2]);
   bool sellSignal = (Close[1] < Low[2]);
   if(!buySignal && !sellSignal) return;

   // V28.07 FIX: Removed Reaper directional gate ? Nexus was blocked whenever Reaper had trades open.
   // Reaper can hold trades for days/weeks, making Nexus permanently disabled.

   double spread = (Ask - Bid) / Point;
   if(spread > InpMax_Spread_Pips * 10) return;

   // V28.07 FIX: Use ATR_median for BOTH SL and TP. Was using compressed ATR for SL (too tight).
   // During compression, atrCurrent is low -> SL gets stopped out before breakout expands.
   double slDist = atrMedian * InpNexus_SL_ATR_Mult;   // Was: atrCurrent (too tight)
...
void UpdateReaperBasketState()
{
   ...
   // V28.07 FIX: Track total lots for proper volume-weighted average
   double buy_total_lots = 0;
   double sell_total_lots = 0;
   for(int i = 0; i < OrdersTotal(); i++)
   {
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
      {
         if(OrderSymbol() == Symbol() && OrderMagicNumber() == InpReaper_BuyMagicNumber)
         {
            g_reaper_buy_levels++;
            buy_trades++;
   ...
   // V28.07 FIX: Divide by total lots (volume-weighted), not trade count (simple average)
   if(buy_trades > 0)
   {
      g_reaper_buy_avg_price /= buy_total_lots;
      g_reaper_buy_active = true;
   }
   
   if(sell_trades > 0)
   {
      g_reaper_sell_avg_price /= sell_total_lots;
...
//+------------------------------------------------------------------+
//| V28.07: ASIAN RANGE BREAKOUT (London Open)                      |
//| Asian session (00:00-08:00 UTC) creates consolidation range.    |
//| London open (08:00 UTC) breaks it. 55-60% directional accuracy  |
//| when combined with D1 trend and Goldilocks range filter.        |
//+------------------------------------------------------------------+
void ExecuteAsianBreakoutStrategy()
{
   // V28.07 LONDON BREAKOUT -- 7-Rule Implementation
   // Rule 1: Mark Asian session high and low (00:00-08:00 UTC)
   // Rule 2: Range must be under 40 pips (400 points)
   ...
   // RULE 1: Mark Asian session high and low
   // On H4: bars at 00:00 and 04:00 UTC = bars [1] and [2] when current bar is 08:00
   int barHour = TimeHour(Time[0]);
   
   // OCO CLEANUP: On any bar after London open, delete unfilled pending orders
   // If we have a filled market order and leftover pending, delete the pending
   if(barHour > 8 || (barHour == 8 && CountOpenTrades(InpAsianBreakout_MagicNumber) > 0))
   {
      // Check for pending orders that should be deleted (OCO Rule 6)
      for(int i = OrdersTotal() - 1; i >= 0; i--)
      {
         if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
         {
            ...
            {
               // If we already have a filled position, delete the pending
               if(CountOpenTrades(InpAsianBreakout_MagicNumber) > 0)
               {
      ...
      // Only place new orders at London open (08:00 UTC bar)
      if(barHour != 8) return;
   }
   ...
   // Don't place if we already have orders from this strategy
   if(CountOpenTrades(InpAsianBreakout_MagicNumber) > 0) return;

   // Get Asian range from previous 2 H4 bars (00:00 and 04:00 UTC)
   double asianHigh = MathMax(High[1], High[2]);
   double asianLow  = MathMin(Low[1], Low[2]);
   double asianRange = asianHigh - asianLow;

   // RULE 2: Range must be under 40 pips (400 points on 5-digit broker)
   double maxRangePips = 40.0;
   double rangeInPips = asianRange / (Point * 10);
   if(rangeInPips > maxRangePips) return;  // Too wide -- skip
   if(rangeInPips < 5.0) return;           // Too narrow -- no edge
   ...
   // Buffer for order placement (just outside Asian range)
   double buffer = 5 * Point; // 0.5 pip buffer

   // RULE 4: SL at opposite side of range
   // Buy: SL below Asian Low, Sell: SL above Asian High
   double buySL  = asianLow - buffer;
   double sellSL = asianHigh + buffer;
   
   // Buy entry: just above Asian High
   double buyEntry  = asianHigh + buffer;
   double buySLdist = buyEntry - buySL;
   
   // Sell entry: just below Asian Low
   double sellEntry  = asianLow - buffer;
   double sellSLdist = sellSL - sellEntry;

   // RULE 5: TP at 1:1.5 risk/reward
   double buyTP  = buyEntry + (buySLdist * 1.5);
   double sellTP = sellEntry - (sellSLdist * 1.5);

   // RULE 3: Place BOTH pending orders simultaneously
   // Place BUYSTOP above Asian High
   int buyTicket = OrderSend(Symbol(), OP_BUYSTOP, lots, buyEntry, 3, buySL, buyTP,
                             "DQ_LONDON_BUY", InpAsianBreakout_MagicNumber, 0, clrGreen);
   
   if(buyTicket > 0)
   {
      if(stratIdx >= 0) g_perfData[stratIdx].trades++;
      LogError(ERROR_INFO, "London Breakout: BUYSTOP placed at " + DoubleToString(buyEntry, 5) +
               " SL=" + DoubleToString(buySL, 5) + " TP=" + DoubleToString(buyTP, 5), 
               "ExecuteAsianBreakoutStrategy");
   }
   
   // Place SELLSTOP below Asian Low
   int sellTicket = OrderSend(Symbol(), OP_SELLSTOP, lots, sellEntry, 3, sellSL, sellTP,
                              "DQ_LONDON_SELL", InpAsianBreakout_MagicNumber, 0, clrRed);
   
   if(sellTicket > 0)
   {
      if(stratIdx >= 0) g_perfData[stratIdx].trades++;
      LogError(ERROR_INFO, "London Breakout: SELLSTOP placed at " + DoubleToString(sellEntry, 5) +
               " SL=" + DoubleToString(sellSL, 5) + " TP=" + DoubleToString(sellTP, 5),
               "ExecuteAsianBreakoutStrategy");
   }
}
...
double Hades_CalculateDynamicBasketTarget(int magic_number)
{
    ...
    double baseTargetProfit = 200.0; // V28.07 FIX: Raised from 50 ? was stealing profits from grids at $5
...
bool Hades_ShouldTakeEarlyProfit(double currentBasketProfit, double dynamicTargetProfit)
{
    ...
    // V28.07 FIX: Require minimum 50% of dynamic target before early exit.
    // OLD BUG: closed baskets at $5 profit on any new equity high ? destroyed grid profits.
    if(currentBasketProfit < dynamicTargetProfit * 0.50)
    {
        return false; // Not enough profit yet ? let the basket breathe
    }
    ...
    // If closing this basket would push our equity to a new all-time high AND we have >= 50% of target...
    if (projectedEquity > g_high_watermark_equity)
    {
        LogError(ERROR_INFO, "HADES Early Exit: New equity high watermark at 50%+ target. Profit: $" + DoubleToString(currentBasketProfit,2));
...
void OnTick_SiliconX()
{
    ...
    {
       // V28.07 FIX: Removed ExecuteSiliconCore() from OnTick ? it's already called from OnNewBar (line 5612).
    // Having it in both places caused duplicate grid management. Basket/trail management stays here (needs every tick).
    // ExecuteSiliconCore(); // REMOVED ? called from OnNewBar with timer guard
    }
}
...
int GetStrategyIndexByMagic(int magicNumber)
{
   ...
   if(magicNumber == InpAsianBreakout_MagicNumber) return 16; // V28.07: Asian Range Breakout
   return -1;
}
...
void CalculateRollingKelly(int idx)
{
   ...
   // ???????????????????????????????????????????????????????
   double payoffRatio = (avgLoss > 0 && avgWin > 0) ? avgWin / avgLoss : 2.0;
   if(payoffRatio <= 0) payoffRatio = 2.0; // V28.07 FIX: Prevent zero divide
