...
int      g_consecLossTracker[22][2];                // [idx][0]=streak(-1=losses,1=wins) [idx][1]=count
datetime g_strategyLockoutUntil[22];                // Per-strategy lockout timestamps
...
StrategyCooldown g_strategyCooldown[22]; // V28.08: Extended to 22 strategies
// ---
...
void UpdatePerformanceMetrics()
{
    ...
    if(total_trades > 10)
    {
        ...
        for(int i = 0; i < 22 && i < ArraySize(g_perfData); i++) // V28.00: Extended to 17
        {
            if(g_perfData[i].trades > 0)
            {
...
extern bool    InpTBM_Enabled               = true;  // Enable Time-Based Manipulation
...
extern bool    InpPO3_Enabled               = true;  // Enable Power of 3
...
extern bool    InpCRT_Enabled               = false;  // Enable CRT Strategy
...
extern double  InpLondonBreakout_MaxRange   = 40.0;       // Max Asian range in pips
extern double  InpLondonBreakout_MinRange   = 5.0;        // Min Asian range in pips
extern double  InpLondonBreakout_RRMultiple = 1.5;        // Risk/Reward multiple (1:1.5)
...
void ReconcileFinalPerformance()
{
   ...
   PerfData reconciledData[22]; // V28.08: Extended to 22 strategies
   for(int i=0; i<22; i++) // V28.00: Extended to 17 strategies
   {
...
void ExecuteLondonBreakoutStrategy()
{
   ...
   if(barHour != 8) return;
   if(CountOpenTrades(InpLondonBreakout_Magic) > 0) return;
   ...
   if(rangeInPips > InpLondonBreakout_MaxRange) return;
   if(rangeInPips < InpLondonBreakout_MinRange) return;
   ...
   // Lot sizing
   double lots = MoneyManagement_Quantum(InpLondonBreakout_Magic, InpBase_Risk_Percent);
...
//+------------------------------------------------------------------+
//| A-TIER STRATEGY 1: Time-Based Manipulation (Umar Arpanjabi)       |
//| H4 bias + Asia session liquidity sweep + BOS entry                |
//+------------------------------------------------------------------+
void ExecuteTimeBasedManipulation(double lotSize, double rrRatio)
{
   Print("[TBM] Called - checking conditions...");
   // Check existing TBM trades
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol()) continue;
      if(OrderMagicNumber() == 9010) { Print("[TBM] Already have open trade, skip"); return; }
   }

   // H4 bias using EMA20
   double ema20 = iMA(Symbol(), PERIOD_H4, 20, 0, MODE_EMA, PRICE_CLOSE, 0);
   int bias = 0;
   if(Close[0] > ema20) bias = 1;
   else if(Close[0] < ema20) bias = -1;
   if(bias == 0) { Print("[TBM] No bias"); return; }
   Print("[TBM] Bias=", bias, " Close=", Close[0], " EMA20=", ema20);

   // TBM: Sweep of previous candle + reversal close
   bool signal = false;
   if(bias == 1 && Low[0] < Low[1] && Close[0] > Open[0] && Close[0] > Low[1]) signal = true;
   if(bias == -1 && High[0] > High[1] && Close[0] < Open[0] && Close[0] < High[1]) signal = true;
   if(!signal) { Print("[TBM] No signal. L0=", Low[0], " L1=", Low[1], " H0=", High[0], " H1=", High[1]); return; }
   Print("[TBM] SIGNAL FOUND! Bias=", bias);

   // Execute trade
   double spread = (Ask - Bid) / Point;
   if(spread > InpMax_Spread_Pips * 10) return;
   double lots = MoneyManagement_Quantum(9010, InpBase_Risk_Percent);
   if(lots < MarketInfo(Symbol(), MODE_MINLOT)) return;

   double buffer = 10 * Point;
   if(bias == 1)
   {
      double sl = Low[0] - buffer;
      double slDist = Ask - sl;
      double tp = Ask + slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, sl, tp, "TBM_Long", 9010, 0, clrGreen);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9010); if(idx >= 0) g_perfData[idx].trades++; }
   }
   else
   {
      double sl = High[0] + buffer;
      double slDist = sl - Bid;
      double tp = Bid - slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_SELL, lots, Bid, 3, sl, tp, "TBM_Short", 9010, 0, clrRed);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9010); if(idx >= 0) g_perfData[idx].trades++; }
   }
}

void ExecutePowerOf3(double lotSize, double rrRatio)
{
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol()) continue;
      if(OrderMagicNumber() == 9011) return;
   }
   ...
   // PO3: Expansion candle (big body, small wicks, in bias direction)
   double totalRange = High[0] - Low[0];
   if(totalRange <= 0) return;
   double body = MathAbs(Close[0] - Open[0]);
   if(body / totalRange < 0.5) return; // Body must be > 50% of range
   if(bias == 1 && Close[0] <= Open[0]) return; // Must be bullish candle
   if(bias == -1 && Close[0] >= Open[0]) return; // Must be bearish candle

   // Execute
   double spread = (Ask - Bid) / Point;
   if(spread > InpMax_Spread_Pips * 10) return;
   double lots = MoneyManagement_Quantum(9011, InpBase_Risk_Percent);
   if(lots < MarketInfo(Symbol(), MODE_MINLOT)) return;
   ...
   if(bias == 1)
   {
      ...
      double slDist = Ask - sl;
      double tp = Ask + slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, sl, tp, "PO3_Long", 9011, 0, clrGreen);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9011); if(idx >= 0) g_perfData[idx].trades++; }
   }
   else
   {
      double sl = High[0] + buffer;
      double slDist = sl - Bid;
      double tp = Bid - slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_SELL, lots, Bid, 3, sl, tp, "PO3_Short", 9011, 0, clrRed);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9011); if(idx >= 0) g_perfData[idx].trades++; }
   }
}
...
void ExecuteFractalModel(double lotSize, double rrRatio)
{
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol()) continue;
      if(OrderMagicNumber() == 9012) return;
   }
   ...
   // Fractal: 3-candle pattern (higher lows for buy, lower highs for sell)
   bool signal = false;
   if(bias == 1 && Low[0] > Low[1] && Low[1] > Low[2] && Close[0] > Open[0]) signal = true;
   if(bias == -1 && High[0] < High[1] && High[1] < High[2] && Close[0] < Open[0]) signal = true;
   if(!signal) return;

   // Execute
   double spread = (Ask - Bid) / Point;
   if(spread > InpMax_Spread_Pips * 10) return;
   double lots = MoneyManagement_Quantum(9012, InpBase_Risk_Percent);
   if(lots < MarketInfo(Symbol(), MODE_MINLOT)) return;

   double buffer = 10 * Point;
   if(bias == 1)
   {
      double sl = Low[2] - buffer;
      double slDist = Ask - sl;
      double tp = Ask + slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, sl, tp, "Fractal_Long", 9012, 0, clrGreen);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9012); if(idx >= 0) g_perfData[idx].trades++; }
   }
   else
   {
      double sl = High[2] + buffer;
      double slDist = sl - Bid;
      double tp = Bid - slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_SELL, lots, Bid, 3, sl, tp, "Fractal_Short", 9012, 0, clrRed);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9012); if(idx >= 0) g_perfData[idx].trades++; }
   }
}

void ExecuteAPlusBreakout(double lotSize, double rrRatio)
{
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol()) continue;
      if(OrderMagicNumber() == 9013) return;
   }

   // Time cutoff (before 17:00 server = 12 PM EST)
   MqlDateTime dt;
   TimeToStruct(TimeCurrent(), dt);
   if(dt.hour >= 17) return;

   // Previous day range
   double rangeHigh = iHigh(Symbol(), PERIOD_D1, 1);
   double rangeLow = iLow(Symbol(), PERIOD_D1, 1);
   double rangeSize = rangeHigh - rangeLow;
   if(rangeSize < 20 * Point) return; // Skip if range too small

   // Trend filter: only trade in direction of H4 EMA
   double ema20 = iMA(Symbol(), PERIOD_H4, 20, 0, MODE_EMA, PRICE_CLOSE, 0);
   double ema50 = iMA(Symbol(), PERIOD_H4, 50, 0, MODE_EMA, PRICE_CLOSE, 0);

   // Detect FVG at breakout
   int fvgBias = 0;
   for(int i = 1; i <= 3; i++)
   {
      if(Low[i-1] > High[i+1] && Close[i] > rangeHigh && Close[0] > ema20 && ema20 > ema50) { fvgBias = 1; break; }
      if(High[i-1] < Low[i+1] && Close[i] < rangeLow && Close[0] < ema20 && ema20 < ema50) { fvgBias = -1; break; }
   }
   if(fvgBias == 0) return;
   ...
   for(int i = 1; i <= 3; i++)
   {
      if(fvgBias == 1 && Low[i-1] > High[i+1]) { fvgHigh = Low[i-1]; fvgLow = High[i+1]; break; }
      if(fvgBias == -1 && High[i-1] < Low[i+1]) { fvgHigh = Low[i+1]; fvgLow = High[i-1]; break; }
   }

   // Check if price is in FVG zone
   if(fvgBias == 1 && (Ask > fvgHigh || Ask < fvgLow)) return;
   if(fvgBias == -1 && (Bid < fvgLow || Bid > fvgHigh)) return;

   // Execute
   double spread = (Ask - Bid) / Point;
   if(spread > InpMax_Spread_Pips * 10) return;
   double lots = MoneyManagement_Quantum(9013, InpBase_Risk_Percent);
   if(lots < MarketInfo(Symbol(), MODE_MINLOT)) return;

   double buffer = 10 * Point;
   if(fvgBias == 1)
   {
      double sl = Low[3] - buffer;
      double slDist = Ask - sl;
      double tp = Ask + slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, sl, tp, "APlus_Long", 9013, 0, clrGreen);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9013); if(idx >= 0) g_perfData[idx].trades++; }
   }
   else
   {
      double sl = High[3] + buffer;
      double slDist = sl - Bid;
      double tp = Bid - slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_SELL, lots, Bid, 3, sl, tp, "APlus_Short", 9013, 0, clrRed);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9013); if(idx >= 0) g_perfData[idx].trades++; }
   }
}

//+------------------------------------------------------------------+
//| A-TIER STRATEGY 5: CRT Candle Range Theory (@Im-speculator)       |
...
void ExecuteCRT(double lotSize, double rrRatio)
{
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol()) continue;
      if(OrderMagicNumber() == 9014) return;
   }

   // Key time check (10:00-12:00 server)
   MqlDateTime dt;
   TimeToStruct(TimeCurrent(), dt);
   // Trade during London+NY sessions
   if(dt.hour < 7 || dt.hour >= 20) return;

   // Bias: EMA alignment on H4
   double ema20 = iMA(Symbol(), PERIOD_H4, 20, 0, MODE_EMA, PRICE_CLOSE, 0);
   double ema50 = iMA(Symbol(), PERIOD_H4, 50, 0, MODE_EMA, PRICE_CLOSE, 0);
   int bias = 0;
   if(Close[0] > ema20 && ema20 > ema50) bias = 1;
   else if(Close[0] < ema20 && ema20 < ema50) bias = -1;
   if(bias == 0) return;

   // Simplified CRT: check if previous H4 candle had a sweep (wick beyond range)
   double prevHigh = High[1], prevLow = Low[1];
   double prev2High = High[2], prev2Low = Low[2];
   bool sweep = false;
   if(bias == 1 && Low[1] < prev2Low && Close[1] > Open[1]) sweep = true; // Sweep low + bullish close
   if(bias == -1 && High[1] > prev2High && Close[1] < Open[1]) sweep = true; // Sweep high + bearish close
   if(!sweep) return;

   // Execute
   double spread = (Ask - Bid) / Point;
   if(spread > InpMax_Spread_Pips * 10) return;
   double lots = MoneyManagement_Quantum(9014, InpBase_Risk_Percent);
   if(lots < MarketInfo(Symbol(), MODE_MINLOT)) return;

   double buffer = 10 * Point;
   double crtHigh = High[1];
   double crtLow = Low[1];

   if(bias == 1)
   {
      double sl = crtLow - buffer;
      double slDist = Ask - sl;
      double tp = Ask + slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, sl, tp, "CRT_Long", 9014, 0, clrGreen);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9014); if(idx >= 0) g_perfData[idx].trades++; }
   }
   else
   {
      double sl = crtHigh + buffer;
      double slDist = sl - Bid;
      double tp = Bid - slDist * rrRatio;
      int ticket = OrderSend(Symbol(), OP_SELL, lots, Bid, 3, sl, tp, "CRT_Short", 9014, 0, clrRed);
      if(ticket > 0) { int idx = GetStrategyIndexFromMagic(9014); if(idx >= 0) g_perfData[idx].trades++; }
   }
}
//+------------------------------------------------------------------+
//| END A-TIER STRATEGIES (V28.08)                                    |
//+------------------------------------------------------------------+
