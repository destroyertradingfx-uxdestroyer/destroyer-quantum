...
//|  DESTROYER QUANTUM V28.09 - VENI VIDI VICI                        |
...
#property version   "28.04"  // V28.04: Cut LiquiditySweep, reverted Titan, fixed lot sizing, adjusted DivergenceMR+StructuralRetest
#property strict
...
extern double  InpPhantom_MaxGap_Pips     = 40.0;  // V28.09: Widened from 30  catch more valid gaps
extern double  InpPhantom_MinGap_Pips     = 3.0;   // V28.09: Lowered from 5  catch smaller gaps
...
// ============================================================
// V28.09: SENTINEL STRATEGY (EMA Crossover + RSI Confirmation)
// ============================================================
sinput string InpSentinel_Header = "====== SENTINEL: EMA CROSSOVER TREND FOLLOWER ======";
extern bool    InpSentinel_Enabled       = true;
extern int     InpSentinel_MagicNumber   = 777015;
extern int     InpSentinel_FastEMA       = 21;        // Fast EMA period
extern int     InpSentinel_SlowEMA       = 50;        // Slow EMA period
extern int     InpSentinel_RSI_Period    = 14;        // RSI period for confirmation
extern double  InpSentinel_RSI_BuyLevel  = 45.0;      // RSI must be above this for BUY
extern double  InpSentinel_RSI_SellLevel = 55.0;      // RSI must be below this for SELL
extern double  InpSentinel_SL_ATR_Mult   = 1.5;       // SL = ATR * this
extern double  InpSentinel_TP_RR         = 2.0;       // TP = SL distance * this (Risk:Reward)
...
PerfData g_perfData[18];  // V28.09: Added Sentinel (index 17)
...
double GetStrategySpecificRisk(int magicNumber)
{
   ...
   // V28.09: THE TREND TIER (Sentinel)
   // EMA crossover trend follower. Conservative 0.8x risk until proven.
...
input double InpNoiseBreakoutATRMult = 0.10;       // V28.09: Reduced from 0.15  catch smaller breakouts
...
// }
   ...
   // V28.09 SENTINEL  EMA Crossover + RSI Confirmation (the "always active" strategy)
...
void ExecuteNoiseBreakout()
{
   ...
   // 3. SQUEEZE DETECTION (shift=2 OR shift=3, V28.09: allow squeeze to persist 2 bars)
   ...
   // V28.09: Also check bar[3]  squeeze can persist for 2 bars before breaking
   double bb_upper_prev2 = iBands(Symbol(), Period(), InpNoiseBB_Period, InpNoiseBB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 3);
   double bb_lower_prev2 = iBands(Symbol(), Period(), InpNoiseBB_Period, InpNoiseBB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 3);
   double kc_atr_prev2 = iATR(Symbol(), Period(), InpNoiseKC_Period, 3);
   double kc_ma_prev2 = iMA(Symbol(), Period(), InpNoiseKC_Period, 0, MODE_SMA, PRICE_TYPICAL, 3);
   double kc_upper_prev2 = kc_ma_prev2 + (kc_atr_prev2 * InpNoiseKC_ATR_Mult);
   double kc_lower_prev2 = kc_ma_prev2 - (kc_atr_prev2 * InpNoiseKC_ATR_Mult);

   bool isSqueezeOn = (bb_upper_prev < kc_upper_prev && bb_lower_prev > kc_lower_prev) ||
                      (bb_upper_prev2 < kc_upper_prev2 && bb_lower_prev2 > kc_lower_prev2);
...
int GetStrategyIndex(int magic)
{
   ...
   if(magic == InpSentinel_MagicNumber) return 17; // V28.09: Sentinel EMA Crossover
   return -1;
}
...
void ExecutePhantomStrategy()
{
   ...
   if(CountOpenTrades(InpPhantom_MagicNumber) > 1) return;   // V28.09: Allow 2 concurrent trades (was 1)
   if(Period() != PERIOD_H4) return;

   // V28.09: Fire on Monday OR Tuesday's first H4 bar (gap fade both days)
   if(DayOfWeek() > 2) return;                              // GATE 5: Mon/Tue only (was Mon only)
   if(TimeHour(TimeCurrent()) > 8) return;                   // V28.09: Extended from 4 to 8
...
//+------------------------------------------------------------------+
//| V28.09: SENTINEL  EMA Crossover + RSI Confirmation              |
//| Magic: 777015                                                    |
//| Frequency: 2-3 trades/week | Target: $100-150/trade             |
//| The "always active" strategy that fills gaps between             |
//| Phantom Mondays and NoiseBreakout squeezes.                      |
//+------------------------------------------------------------------+
...
int CheckDirectionalBias()
{
   ...
   // BUY allowed: price above 200 EMA (or within 1.0% = near support)  V28.09: widened from 0.5%
   bool buyAllowed = (distPct >= -0.010);
   
   // SELL allowed: price below 200 EMA (or within 1.0% = near resistance)  V28.09: widened from 0.5%
   bool sellAllowed = (distPct <= 0.010);
