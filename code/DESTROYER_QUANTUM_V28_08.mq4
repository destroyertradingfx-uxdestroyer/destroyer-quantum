...
//|  DESTROYER QUANTUM V28.08 - VENI VIDI VICI                        |
...
extern int     InpTradingStartHour     = 7;           // V28.08: Extended from 8  catch London pre-open
extern int     InpTradingEndHour       = 20;          // V28.08: Extended from 18  catch US close
...
extern double InpReaper_BasketSL_Money  = 800.0;     // V28.08: Basket stop-loss  max loss per grid before forced close
...
double GetStrategySpecificRisk(int magicNumber)
{
   ...
   // V28.08: NoiseBreakout boosted from 0.3x to 0.45x  consistent performer deserves more capital
   if(magicNumber == InpWarden_MagicNumber || magicNumber == 777012) // 777009 or 777012
      return 0.45; 

   // V28.08: THE WORKHORSE TIER (Phantom)
   // Phantom is the #1 profit generator. 166 trades, $24K, PF 1.71. Give it 1.2x risk.
   if(magicNumber == 777013) // Phantom - Monday Gap Fader
      return 1.2;
...
void ProcessReaperBasket(int magic_number, int order_type)
{
   ...
   // V28.08 FIX #3: BASKET STOP-LOSS  cap the bleeding on losing grids
   // Without this, a losing grid can hemorrhage $2,000+ before hitting daily loss limit
   if(basket_profit <= -InpReaper_BasketSL_Money)
   {
      CloseAllByMagic(magic_number);
      LogError(ERROR_WARNING, "Reaper Basket STOP-LOSS HIT -$" + DoubleToString(InpReaper_BasketSL_Money, 0) + 
                "! Actual Loss: $" + DoubleToString(basket_profit, 2) + 
                " (levels: " + IntegerToString(basket_levels) + ") - CUTTING LOSSES", "ProcessReaperBasket");
      return;
   }
